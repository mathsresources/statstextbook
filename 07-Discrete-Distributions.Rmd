...
\begin{itemize}
    ...
    \item $X$ represents the \textbf{number of successes} in a \textit{fixed} number of trials, $n$.
...
\begin{itemize}
    ...
    \item $X$ represents the \textbf{number of events occurring} in a set amount of time or space.
...
\pgfplotsset{
    ...
    \node[fill=white] at (9,3.5) {one hour, $\lambda=2.5$};
...
\begin{tikzpicture}
    ...
    \draw [decorate,decoration={brace,amplitude=5pt,raise=1ex}]
  (3*\x-3+0.1,0.8) -- (3*\x-3+2.8,0.8) node[midway,above,yshift=0.3cm]{$X_\x \sim \text{Po}\left(3\right)$};
    }
