...
\begin{enumerate}
    ...
    \item A kettle manufacturer wishes to test whether the mean time for its kettles to boil one litre of water is more than 65 seconds stated on the packaging.
    ...
    \item A music fan measures the tempo (in beats per minute) of a sample of songs on a streaming platform to test if a suggested mean of 160 beats per minute is too low.
...
If $p\text{-value}\leqslant\alpha$ then \textbf{reject }$\mathbf{\text{H}_\text{0}}$ and conclude that\\ there is \textit{sufficient evidence to suggest} that the  alternative hypothesis is true.\\
\vspace{0.5cm}
If $p\text{-value}>\alpha$ then \textbf{do not reject }$\mathbf{\text{H}_\text{0}}$ and conclude that\\ there is \textit{insufficient evidence to suggest} that the alternative hypothesis is true.
\end{tcolorbox}
\end{centering}
...
Since $0.3085>0.05$, accept $\text{H}_\text{0}$ at the 5\% level of significance. There is insufficient evidence to suggest that the mass of dried tea leaves in a bag bag is less than 2.5 grams.

\end{tcolorbox}
The hypotheses given by the student lack the required clarity.
\begin{enumerate}
    \item State suitable hypotheses which should be used instead.\\[-0.6em]
\end{enumerate}
The $p$-value calculated by the student is correct, but it lacks sufficient working.
\begin{enumerate}[resume]
    \item Show a minimum of two additional steps of working, which lead to the value of $0.3085$.\\[-0.6em]
\end{enumerate}
...
Obtaining a $p$-value in the previous section required calculating the probability of the observed sample mean, $\bar{x}$, by first using the $z$*-transformation for a sample mean* to obtain what is called a $z$*-score*. Instead of concluding a $z$-test by comparing a $p$-value to the significance level, an alternative approach is to compare the $z$-score, referred to as the **test statistic**, to pre-determined cut-off values, called **critical values**.
...
*Critical values* can be found by first considering *critical regions*. These are the most extreme regions of the sampling distribution of the test statistic; if the calculated test statistic lies within these regions, the null hypothesis is rejected.

Critical values are the *boundaries* of these critical regions. Below are examples of critical regions (shaded), and critical values (below the axis), for a $z$ test statistic with a 5\% level of significance.
...
\begin{centering}
\tcbset{width=17cm}
\begin{tcolorbox}[adjusted title=The decision rule for test statistics and critical values for parametric tests:, halign title = center,colback=red!5,colframe=red!70!black, halign = center]
If the \textbf{test statistic is further from zero than the critical value(s)} then \textbf{reject }$\mathbf{\text{H}_\text{0}}$\\ and conclude that there is \textit{sufficient evidence to suggest} that the  alternative hypothesis is true.\\
\vspace{0.5cm}
If the \textbf{test statistic is not further from zero than the critical value(s)} then \textbf{do not reject }$\mathbf{\text{H}_\text{0}}$\\ and conclude that there is \textit{insufficient evidence to suggest} that the alternative hypothesis is true.
\end{tcolorbox}
\end{centering}

This approach is equivalent to the $p$-value method and will always lead to the same conclusion.

One need for this approach is that it can be difficult to calculate $p$-values for some distributions. The Data Booklet provides common critical values for a range of tests without the need for more extensive probability tables or a graphical calculator. For the remainder of this textbook, the predominant approach used in examples will be to consider *test statistics and critical values*.
...
When choosing a critical value for a two-tailed test, in which there are two critical regions, the critical value whose sign (positive or negative) matches the observed test statistic should be used in the conclusion.
...
\begin{enumerate}[resume]
    ...
    \item A large school plans to test whether the mean amount its teachers spend on photocopying is different from the £12 per year they currently budget for. A random sample of 23 teachers gives a mean of £10.90 and a standard deviation of £2.30. 
    \begin{enumerate}
...
Since $-1.127>-2.998$, do not reject $\text{H}_\text{0}$ at the 1\% level of significance. There is insufficient evidence to suggest that the mean mass of crisps in a bag is less than the stated value of 150g.
...
Since $3.749>2.447$, reject $\text{H}_\text{0}$ at the 5\% level of significance. There is sufficient evidence to suggest that the mean mark students will achieve is no longer 72, and that the difficulty of the exam has changed.
...
Since $3.76>2.58$, reject $\text{H}_\text{0}$ at the 1\% level of significance.\\ There is sufficient evidence to suggest that the true proportion of voters intending to vote for that candidate is not 85\%, and that the candidates claim may be untrue.
