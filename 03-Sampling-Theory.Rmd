...
When information is collected from an *entire population*, a **census** of the population has been taken. It is usually very difficult to carry out a census as it is time consuming, expensive, and requires an accurate and complete list of every member of the population. Collecting data can also be destructive (such as testing the burn time of candles) or involve risk (such as trialling an experimental drug).

It is therefore almost always the case that statisticians work with a **sample**, collecting information from just *part* of a population. Where appropriate processes are used with the aim of producing a sample which is *representative* of the population, it should give an accurate indication of the characteristics of the population and allow statistical inference techniques to be used.
...
Parameters, such as the population mean, are fixed but usually unknown, while sample statistics, such as the sample mean, vary from sample to sample. In other words, if samples are repeatedly taken from a population and their means are obtained, the values of those sample means will sometimes be higher than that of the true population mean and sometimes lower. This comes from *random sampling error* which, despite its name, is a natural effect due to chance variation.

The figure below shows two sets of sample means for random samples taken from the same distribution with a population mean of $\mu=\text{50}$. For one set a sample size of $n=\text{5}$ was used, with $n=\text{20}$ for the other.
...
\begin{tikzpicture}
        ...
        \node at (4,3.4) {population mean};
        \draw[thick] (4,0) -- (4,2.6) node[above] {$\mu$};
        ...
        \draw[stealth-] (7.5,1) -- (8.5,1) node[right] {ten sample means $\bar{x}_i$ with $n=\text{5}$};
        \node[left] at  (33*0.2-6,2) {\textit{larger sample size}};
        \node[left] at  (33*0.2-6,1) {\textit{smaller sample size}};
        ...
        \draw[stealth-] (7.5,2) -- (8.5,2) node[right] {ten sample means $\bar{x}_i$ with $n=\text{20}$};
\end{tikzpicture}
...
\begin{center}
    \begin{tikzpicture}
        \draw (0,0) -- (5,0);
        \draw[dashed] (0.6,0) -- (0.6,1) node[above] at (0.6,1) {${\scriptstyle \bar{x}_\text{1}}$};
        \draw[dashed] (2.9,0) -- (2.9,1) node[above] at (2.9,1) {${\scriptstyle \bar{x}_\text{2}}$};
        \draw[dashed] (1,0) -- (1,1) node[above] at (1,1) {${\scriptstyle \bar{x}_\text{3}}$};
        \draw[dashed] (0.1,0) -- (0.1,1) node[above] at (0.1,1) {${\scriptstyle \bar{x}_\text{4}}$};
        \draw[dashed] (2,0) -- (2,1) node[above] at (2,1) {${\scriptstyle \bar{x}_\text{5}}$};
        \draw[thick] (2.5,-0.2) -- (2.5,1.5) node[above] at (2.5,1.5) {$\mu$};
        \node at (2.5,-0.5) {\textit{biased} (tending to underestimate)};
        \draw (6,0) -- (11,0);
        \draw[dashed] (8.8,0) -- (8.8,1) node[above] at (8.8,1) {${\scriptstyle \bar{x}_\text{1}}$};
        \draw[dashed] (10.1,0) -- (10.1,1) node[above] at (10.1,1) {${\scriptstyle \bar{x}_\text{2}}$};
        \draw[dashed] (6.6,0) -- (6.6,1) node[above] at (6.6,1) {${\scriptstyle \bar{x}_\text{3}}$};
        \draw[dashed] (8,0) -- (8,1) node[above] at (8,1) {${\scriptstyle \bar{x}_\text{4}}$};
        \draw[dashed] (9.2,0) -- (9.2,1) node[above] at (9.2,1) {${\scriptstyle \bar{x}_\text{5}}$};
        \draw[thick] (8.5,-0.2) -- (8.5,1.5) node[above] at (8.5,1.5) {$\mu$};
        \node at (8.5,-0.5) {\textit{unbiased} (accurate estimation)};
        \draw (12,0) -- (17,0);
        \draw[dashed] (16.8,0) -- (16.8,1) node[above] at (16.8,1) {${\scriptstyle \bar{x}_\text{1}}$};
        \draw[dashed] (15,0) -- (15,1) node[above] at (15,1) {${\scriptstyle \bar{x}_\text{2}}$};
        \draw[dashed] (14,0) -- (14,1) node[above] at (14,1) {${\scriptstyle \bar{x}_\text{3}}$};
        \draw[dashed] (16.3,0) -- (16.3,1) node[above] at (16.3,1) {${\scriptstyle \bar{x}_\text{4}}$};
        \draw[dashed] (15.7,0) -- (15.7,1) node[above] at (15.7,1) {${\scriptstyle \bar{x}_\text{5}}$};
        \draw[thick] (14.5,-0.2) -- (14.5,1.5) node[above] at (14.5,1.5) {$\mu$};
        \node at (14.5,-0.5) {\textit{biased} (tending to overestimate)};
    \end{tikzpicture}
\end{center}

It can be very difficult to identify that bias has been introduced, and equally difficult to correct it in practice. A key strategy for minimising the introduction of bias is through the application of *appropriate random sampling techniques*.
...
The \textbf{sampling frame} is the list or collection of sampling units from which the sample is actually drawn, of size $N$. Ideally, the sampling frame should closely match the target population.

\textit{e.g. a numbered list of the school roll}

\vspace{0.2cm}

Sampling frames consist of \textbf{sampling units} which may be selected to form a sample. These may be people, buildings, square grids on a map, and so on. 

\textit{e.g. those pupils listed on the school roll}
...
The \textbf{sampling method} is the way in which sampling units are selected from the sampling frame to create the sample. Sampling methods should feature a \textit{random process} to reduce potential bias.
...
An ill-defined target population, an unrepresentative sampling frame or a poor sampling technique is likely to produce data which is *biased* and *not representative of the population*, leading to *unreliable conclusions*.
...
\begin{enumerate}
    \item A maths teacher conducts a survey of pupils' confidence in the subject by asking the first twenty pupils who enter the maths corridor at the start of first period. Suggest one issue with the \textbf{sampling frame} and one issue with the \textbf{sampling method}, explaining their potential impact.
    ...
    \begin{enumerate}
        \item Identify: \qquad (i) The target population. \qquad (ii) The sampling frame.
        \item Suggest two problems with the \textbf{sampling frame}, and explain the impact this may have on their data.
        \item Comment on the suitability of the \textbf{sampling method} the researcher plans to use.
    \end{enumerate}
    ...
    \begin{enumerate}
        \item Identify one problem with a proposal to use individual flowers as sampling units.
...
\begin{tikzpicture}
\draw[semithick,fill=black!10] (0,0) rectangle (8,5);
\node at (4,5.3) {Sampling Frame, $N=\text{120}$};
\draw[semithick,fill=black!10] (12,2.5) circle (1.5);
\node at (12,4.3) {Sample, $n=\text{15}$};
...
*Systematic sampling* involves using an orderly *pattern* to select units, starting from a randomly selected point. In general, for a sample of size $n$ from a numbered sampling frame of size $N$, firstly a random number generator may select a number from 1 to $k$, where $k=\frac{N}{n}$, with the corresponding sampling unit selected. After that initial selection, *every subsequent* $k\text{th}$ *unit is selected* to complete the sample of size $n$.

For example, a systematic sample of 15 pupils from a year group of 120 can be made by first selecting a random starting point from 1 to $\frac{\text{120}}{\text{15}}=8$, such as the  3rd pupil, in an alphabetised list of pupils in the year group. From there, every $\text{8}$th pupil from the list is chosen until 15 pupils have been chosen.
...
For example, a hospital wishing to survey its staff about the food in the staff canteen may split the $N=\text{120}$ staff into management, doctors/nurses and support staff. For a sample of size $n=\text{12}$, covering $\frac{\text{1}}{\text{10}}$th of the sampling frame, simple random sampling is used to select $\frac{\text{1}}{\text{10}}$ of the staff *from each group*. These are then collected to form the stratified sample. 

\begin{center}
\begin{tikzpicture}
\draw[semithick,fill=black!10] (0,0) rectangle (8,5);
\node at (4,5.3) {Stratified Sampling Frames, $N=\text{120}$};
\draw[semithick,fill=black!10] (12,2.5) ellipse (1 and 2);
\node[above] at (12,4.5) {Sample, $n=\text{12}$};
...
\node[below] at (9.25,2.5) {\small{$\frac{\text{1}}{\text{10}}$ of each}};
...
Like stratified sampling, *quota sampling* considers subgroups within the population. However, it *does not use a random process*, and the number sampled from each stratum often comes from a set *quota* rather than being proportional to their size. 
...
For example, interviewers for a marketing company at a shopping centre speak to people who pass, choosing for themselves who to interview from their allocated strata, with no random process used to avoid potential bias.
...
\begin{itemize}
    \item The time and cost involved in planning and carrying it out (i.e. quick or slow, cheap or expensive).
    \item The ease with which it can be administered (such as whether access to the sampling frame(s) is straightforward).
    \item How reliably it may produce a sample which is representative of the population (such as stratified to minimise chances of a subsection of the population being overlooked in a sample).
\end{itemize}
...
\begin{enumerate}
  ...
  \begin{enumerate}
  \item The population is made up of groups, each of which contains broadly similar characteristics to each other, and the population. One or more of these groups are randomly selected, and everyone in those selected groups is chosen to make up the sample.
  ...
  \item Once an initial random individual from the population has been selected, subsequent individuals are selected at regular intervals until a sample has been obtained.
  ...
  \end{enumerate}
  \item The operators of a cruise ship wish to survey the condition of the fittings in the cabins on board by carefully examining a sample of rooms. Of the 360 cabins, 24 are classed as \textit{Deluxe}, 72 are \textit{Premium} and the remaining are \textit{Standard}.
\begin{enumerate}
