...
\begin{centering}
\tcbset{width=13cm}
\begin{tcolorbox}[adjusted title=Conditions for valid use of the one-sample $z$-test for the population mean:, halign title = center,colback=red!5,colframe=red!70!black, halign = center,center]
\begin{itemize}
    \item The population is \textbf{normally distributed} (or $n>20$).
...
\begin{enumerate}
    ...
    \end{tabular}
    \end{center}
    It is assumed that the standard deviation is a reasonable estimate for the population standard deviation.\newline
...
The calculations involved in calculating a *two-tailed* $z$-confidence interval for the population mean correspond with those for a *two-tailed* $z$-test for the population mean. Such a confidence interval could therefore allow inferences to be made about the population mean.
...
If $\mu_0$ is \textbf{outwith a calculated 95\% confidence interval} then this would provide evidence at the 5\% significance level to suggest that the true population mean is different to $\mu_0$.\\
\vspace{0.5cm}
If $\mu_0$ is \textbf{within a calculated 95\% confidence interval} then this would be insufficient evidence at the 5\% significance level to suggest that the true population mean is different to $\mu_0$.\\
\end{tcolorbox}
\end{centering}
...
Since any confidence intervals encountered in this course will be two-tailed, it is only appropriate to comment on *two-tailed* conclusions (i.e. ``difference...'') when using language such as *`significance'* and *`evidence'*. If a one-tailed comment is needed, it is advised to use less formal language.
...
\begin{enumerate}[itemsep=16pt]
    ...
    \vspace{0.3cm}
    \begin{enumerate}
        \item State an assumption required in order to calculate a $z$-confidence interval for the mean mass a tripod can withstand before failure.
