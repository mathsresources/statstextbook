...
Given a *random sample* from a distribution that is *normally distributed* with *known population standard deviation* $\sigma$, a $\text{100}(\text{1}-\alpha)\%$ $z$-*confidence interval* for the population mean, $\mu$, can be calculated:

\begin{centering}
\tcbset{width=10cm}
\begin{tcolorbox}[adjusted title=A $z$-confidence interval for the population mean:, halign title = center,colback=red!5,colframe=red!70!black, halign = center]
A $\text{100}(\text{1}-\alpha)\%$ CI for $\mu$ is: $\bar{x}\pm z_{\text{1}-\alpha/\text{2}} \times \cfrac{\sigma}{\sqrt{n}}$
\end{tcolorbox}
\end{centering}

As a reminder, the quantity $\dfrac{\sigma}{\sqrt{n}}$ is called the *standard error of the sample mean*.

\begin{centering}
\tcbset{width=15cm}
\begin{tcolorbox}[adjusted title=Conditions for a $z$-confidence interval for the population mean:, halign title = center,colback=red!5,colframe=red!70!black, halign = center,center]
\begin{itemize}
    \item The population is \textbf{normally distributed}, or the sample size is sufficiently large ($n>\text{20}$) such that the Central Limit Theorem applies.
    \item The \textbf{population standard deviation}, $\sigma$, is known (and has not changed).
    \item The observed data are \textbf{independent} (obtained through a random sample).
\end{itemize}
\end{tcolorbox}
\end{centering}

For large sample sizes, where the population standard deviation $\sigma$ is not known, it may be decided that the sample standard deviation is a reasonable estimate ($\sigma \approx s$). In practice, the $t$-distribution is generally preferred when only the sample standard deviation is known, and hence $t$-confidence intervals for the population mean, introduced in the next section, are far more commonly encountered.

Whilst the notation contained in the formula for a confidence interval can look complicated, it should be noted that the $z$-value for a two-tailed 95\% confidence interval corresponds with that for the critical value of a two-tailed $z$-test at the 5\% level of significance. A sketch can be used to help find the required $z$-value, just as in the previous chapter. 
Although *one-tailed* confidence intervals exist, in this course only *two-tailed* confidence intervals will be calculated.
...
\tcblower
\textbf{Solution:}
$\bar{x}=\text{430}$, $n=\text{15}$, $\sigma=\text{12}$\\
...
A 95\% CI for $\mu$ is: \hspace{0.5cm}$\text{430} \pm \text{1.96} \times \cfrac{\text{12}}{\sqrt{\text{15}}}$\hspace{0.5cm}=\hspace{0.5cm}$\text{430} \pm \text{6.07}$\\

Therefore a 95\% confidence interval for the population mean dish mass, in grams, is $(\text{423.93} , \text{436.07})$.\\
...
\begin{enumerate}
    ...
    \begin{center}
    \begin{tabular}{c c c} 
      $n=\text{25}$ & $\Sigma x=\text{1254}$ & $\Sigma x^\text{2}=\text{63186}$\\ 
    \end{tabular}
    \end{center}
    ...
    \item A workers' rights organisation wishes to understand the number of hours worked per week by full-time employees of the local council. A random sample of 24 employees gives a sample mean of 38.2 hours and a standard deviation of 1.4 hours. It is decided that the standard deviation obtained is a reasonable estimate for the population standard deviation.
    \begin{enumerate}
...
Consider the following hypotheses, where $\mu_\text{0}$ is a possible value for a population mean of interest:

\begin{center}
\begin{math}
\begin{array}{ll}
      \text{H}_\text{0}: & \mu = \mu_\text{0}\\[0.5em]
      \text{H}_\text{1}: & \mu \ne \mu_\text{0} 
    \end{array}
\end{math}
\end{center}
...
\begin{centering}
\tcbset{width=16cm}
\begin{tcolorbox}[adjusted title=Inference using confidence intervals:, halign title = center,colback=red!5,colframe=red!70!black, halign = center]
If $\mu_\text{0}$ is \textbf{outwith a calculated 95\% confidence interval} then there is evidence at the 5\% significance level to suggest that $\mu \ne \mu_\text{0}$.\\
\vspace{0.5cm}
If $\mu_\text{0}$ is \textbf{within a calculated 95\% confidence interval} then there is insufficient evidence at the 5\% significance level to suggest that $\mu \ne \mu_\text{0}$.\\
\end{tcolorbox}
\end{centering}

It should be noted that, in this course, an instruction to perform a \textit{test} requires a hypothesis test to be conducted using either a $p$-value or a test statistic, rather than a confidence interval.
...
\tcblower
\textbf{Solution:}
$\bar{x}=\text{273}$, $n=\text{14}$, $\sigma=\text{13}$\\
...
\vspace{0.5cm}
A 95\% CI for $\mu$ is: \hspace{0.5cm}$\text{273} \pm \text{1.96} \times \cfrac{\text{13}}{\sqrt{\text{14}}}$\hspace{0.5cm}=\hspace{0.5cm}$\text{273} \pm \text{6.81}$\\

\vspace{0.1cm}

Therefore a 95\% confidence interval for the mean mass of food now being eaten daily by their cat, in grams, is $(\text{266.19} , \text{279.81})$.\\
...
\begin{enumerate}[itemsep=16pt]
    \item A packing process packages coffee beans into bags. The mass of each bag is normally distributed with standard deviation 15g. A random sample of 25 bags is taken and the sample mean is found to be 493g. Obtain a 95\% confidence interval for the mean mass of coffee beans in a bag, and comment on the claim that the mean mass being dispensed is not the 500g that it is supposed to be.
    ...
    \begin{enumerate}
        ...
        \item Comment on the concern that the mean bar length produced by the machine is no longer the 420cm that it was prior to it being serviced.
    \end{enumerate}
...
\begin{centering}
\tcbset{width=10cm}
\begin{tcolorbox}[adjusted title=A $t$-confidence interval for the population mean:, halign title = center,colback=red!5,colframe=red!70!black, halign = center]
A $\text{100}(\text{1}-\alpha)\%$ CI for $\mu$ is: $\bar{x}\pm t_{n-\text{1},\text{1}-\alpha/\text{2}} \times \cfrac{s}{\sqrt{n}}$
\end{tcolorbox}
\end{centering}
...
\begin{centering}
\tcbset{width=15cm}
\begin{tcolorbox}[adjusted title=Conditions for a $t$-confidence interval for the population mean:, halign title = center,colback=red!5,colframe=red!70!black, halign = center,center]
\begin{itemize}
    \item The population is \textbf{normally distributed}.
    \item The observed data are \textbf{independent} (obtained through a random sample).
\end{itemize}
\end{tcolorbox}
\end{centering}
...
\tcblower
\textbf{Solution:}
$\bar{x}=\text{8.729}$, $n=\text{7}$, $s=\text{1.055}$\\
...
\node[below] at (axis cs: -2.5, 0) {$-\text{3.707}$};
...
\vspace{0.5cm}
A 99\% CI for $\mu$ is: \hspace{0.5cm}$\text{8.729} \pm \text{3.707} \times \cfrac{\text{1.055}}{\sqrt{\text{7}}}$\hspace{0.5cm}=\hspace{0.5cm}$\text{8.729} \pm \text{1.478}$\\

\vspace{0.1cm}

Therefore a 99\% CI for the mean daily electricity usage of mid-terrace houses in Fife, in kWh, is $(\text{7.251} , \text{10.207})$.\\
...
\begin{enumerate}[itemsep=16pt]
    ...
    \begin{center}
    \begin{tabular}{c c c} 
      $n=\text{10}$ & $\Sigma x=\text{9920}$ & $\Sigma x^\text{2}=\text{9868100}$\\ 
    \end{tabular}
    \end{center}
    ...
    \vspace{0.3cm}
    \begin{enumerate}
        \item State an assumption required in order to calculate a $t$-confidence interval for the mean mass a tripod can withstand before failure.
...
As introduced in chapter 10, the sample proportion, $\hat{P}$, has standard error: $\sqrt{\dfrac{pq}{n}}$.

When calculating a $z$-confidence interval for the population proportion, it is necessary to instead use the \textit{sample proportion}, $\hat{p}$, as an \textit{estimate} for $p$, leading to an **estimated standard error**.

$\text{Estimated standard error of the sample proportion: }\quad\sqrt{\dfrac{\hat{p}\hat{q}}{n}}$.

\begin{centering}
\tcbset{width=10cm}
\begin{tcolorbox}[adjusted title=A $z$-confidence interval for the population proportion:, halign title = center,colback=red!5,colframe=red!70!black, halign = center]
A $\text{100}(\text{1}-\alpha)\%$ CI for $p$ is: $\hat{p}\pm z_{\text{1}-\alpha/\text{2}} \times \sqrt{\cfrac{\hat{p}\hat{q}}{n}}$
\end{tcolorbox}
\end{centering}
...
\begin{centering}
\tcbset{width=15cm}
\begin{tcolorbox}[adjusted title=Conditions for a $z$-confidence interval for the population proportion:, halign title = center,colback=red!5,colframe=red!70!black, halign = center,center]
\begin{itemize}
    \item Both $n\hat{p}>\text{5}$ and $n\hat{q}>\text{5}$.
    \item The observed data are \textbf{independent} (obtained through a random sample).
\end{itemize}
\end{tcolorbox}
\end{centering}
...
\tcblower
\textbf{Solution:}
$\hat{p}=\frac{\text{41}}{\text{50}}=\text{0.82}$, $\hat{q}=\text{0.18}$, $n=\text{50}$\\
...
A 95\% CI for $p$ is: \hspace{0.5cm} $\text{0.82} \pm \text{1.96} \times \sqrt{\cfrac{\text{0.82}\times\text{0.18}}{\text{50}}}$\hspace{0.5cm}=\hspace{0.5cm}$\text{0.82} \pm \text{0.060}$\\

\vspace{0.1cm}

Therefore an approximate 95\% confidence interval for the proportion of the website's readers that have changed their router's password is $(\text{0.760} , \text{0.880})$\\
...
\begin{enumerate}[itemsep=18pt]
    \item A newspaper conducts a survey to explore the proportion of its readers that are car owners. In a random sample of 312 readers there are 208 car owners. Obtain an approximate 99\% confidence interval for the proportion of car owners amongst the newspaper's readers.
    \item A sample of 25 pupils in a school are asked whether they prefer cats or dogs, and only 7 prefer cats.
    \begin{enumerate}
        \item State an assumption required about the sample of pupils.
        \item Obtain an approximate 90\% confidence interval for the proportion of pupils in the school that prefer cats.
        \item Show the calculations required to verify that this data is suitable for a $z$-confidence interval.
    \end{enumerate}
    \item In an opinion poll undertaken by the Scottish government, 1000 randomly chosen adults living in Scotland were asked whether they would support a proposed ban on advertising junk food on TV, and 643 replied 'yes'. Obtain an approximate 95\% confidence interval for the true proportion of adults in Scotland who would support the ban.
...
\begin{enumerate}
    ...
    \begin{enumerate}
        ...
        \item A repeat customer claims that the mean volume served is not the correct measure of 125ml, as stated on the restaurant's wine list. Comment on this claim with reference to the confidence interval.
    \end{enumerate}
    ...
    \begin{enumerate}
        \item Determine the value of the standard deviation used to calculate the confidence interval.
    \end{enumerate}
    \item A driving instructor posts an advert online stating nine out of ten of their students pass their driving test first time. For a random sample of students taught by the instructor, the table below shows how many of them passed their driving test first time, and how many did not pass the first time. 
    \begin{center}
            \begin{tabular}{|c|c|}
                \hline
                \textbf{Result of first driving test} & \textbf{Frequency}\\
                \hline
                Pass & 47\\
                \hline
                Fail & 13\\
