...
Given a *random sample* from a distribution that is *normally distributed* with *known population standard deviation* $\sigma$, a $100(1-\alpha)\%$ $z$-*confidence interval* for the population mean, $\mu$, can be calculated:
...
A $100(1-\alpha)\%$ CI for $\mu$ is: $\bar{x}\pm z_{1-\alpha/2} \times \cfrac{\sigma}{\sqrt{n}}$
\end{tcolorbox}
\end{centering}
...
\begin{itemize}
    \item The population is \textbf{normally distributed}, or the sample size is sufficiently large ($n>20$) such that the Central Limit Theorem applies.
...
Consider the following hypotheses, where $\mu_0$ is a possible value for a population mean of interest:

\begin{center}
\begin{math}
\begin{array}{ll}
      \text{H}_0: & \mu = \mu_0\\[0.5em]
      \text{H}_1: & \mu \ne \mu_0 
    \end{array}
\end{math}
\end{center}
...
If $\mu_0$ is \textbf{outwith a calculated 95\% confidence interval} then there is evidence at the 5\% significance level to suggest that $\mu \ne \mu_0$.\\
\vspace{0.5cm}
If $\mu_0$ is \textbf{within a calculated 95\% confidence interval} then there is insufficient evidence at the 5\% significance level to suggest that $\mu \ne \mu_0$.\\
\end{tcolorbox}
\end{centering}
...
A $100(1-\alpha)\%$ CI for $\mu$ is: $\bar{x}\pm t_{n-1,1-\alpha/2} \times \cfrac{s}{\sqrt{n}}$
\end{tcolorbox}
\end{centering}
...
A $100(1-\alpha)\%$ CI for $p$ is: $\hat{p}\pm z_{1-\alpha/2} \times \sqrt{\cfrac{\hat{p}\hat{q}}{n}}$
\end{tcolorbox}
\end{centering}
...
\begin{itemize}
    \item Both $n\hat{p}>5$ and $n\hat{q}>5$.
    \item The observed data are \textbf{independent} (obtained through a random sample).
\end{itemize}
\end{tcolorbox}
\end{centering}
