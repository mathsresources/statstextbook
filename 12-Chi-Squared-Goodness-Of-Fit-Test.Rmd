...
The **chi-squared distribution**, $\chi^{\text{2}}_\nu$, describes the distribution of *sums* of *independent squared and scaled random variables*. \mbox{$\chi^{\text{2}}$ hypothesis tests} offer a way to analyse categorical data, and are a type of **non-parametric** test.
...
The $\chi^{\text{2}}$ goodness-of-fit test asks whether the observed frequencies are so *significantly different* from the expected frequencies, under the model described in the null hypothesis, that there is evidence to suggest *the model is in fact incorrect*.

\begin{center}
\begin{tabular}{r c l}
    $\text{H}_\text{0}:$ & \quad & The score on the die follows a uniform distribution (the die is fair).\\
    $\text{H}_\text{1}:$ & \quad & The score on the die follows some other distribution (the die is not fair).
\end{tabular}
\end{center}
...
\begin{centering}
\tcbset{width=15cm}
\begin{tcolorbox}[adjusted title=For a $\chi^{\text{2}}$ goodness-of-fit test:, halign title = center,colback=red!5,colframe=red!70!black, halign = center]
Test statistic: $\chi^{\text{2}}=\cfrac{(O_\text{1}-E_\text{1})^\text{2}}{E_\text{1}}+\cfrac{(O_\text{2}-E_\text{2})^\text{2}}{E_\text{2}}+...+\cfrac{(O_n-E_n)^\text{2}}{E_n}=\sum{\cfrac{(O_i-E_i)^\text{2}}{E_i}}$
\end{tcolorbox}
\end{centering}

The test statistic follows a $\chi^{\text{2}}_\nu$ distribution with degrees of freedom $\nu=c-\text{1}-m$, where:
...
\begin{minipage}[c]{.54\textwidth}
  ...
  \begin{itemize}
    ...
    \item $m$ is the number of \textit{model parameters}\newline estimated using the observed data\newline (Such as estimating $\lambda$ for a $\text{Po}(\lambda)$ distribution).
\end{itemize}

\vspace{0.3cm}

\begin{centering}
\begin{tcolorbox}[adjusted title=The decision rule for $\chi^{\text{2}}$ tests:, halign title = center,colback=red!5,colframe=red!70!black, halign = center]
If \textbf{test statistic} $>$ \textbf{critical value}, \textit{reject }$\text{H}_\text{0}$.\\
\vspace{0.5cm}
If \textbf{test statistic} $\leqslant$ \textbf{critical value}, \textit{do not reject }$\text{H}_\text{0}$.\\
\end{tcolorbox}
\end{centering}
\end{minipage}
\end{figure}
...
$$\chi^{\text{2}}=\frac{(\text{7}-\text{8})^\text{2}}{\text{8}}+\frac{(\text{8}-\text{8})^\text{2}}{\text{8}}+\frac{(\text{5}-\text{8})^\text{2}}{\text{8}}+\frac{(\text{10}-\text{8})^\text{2}}{\text{8}}+\frac{(\text{7}-\text{8})^\text{2}}{\text{8}}+\frac{(\text{11}-\text{8})^\text{2}}{\text{8}}=\text{3}$$

\vspace{-0.5cm}

There are six categories, so $c=\text{6}$, and no parameters of the model were calculated using the data, so $m=\text{0}$, leading to $\nu=\text{6}-\text{1}-\text{0}=\text{5}$ degrees of freedom. This corresponds to the number of observed frequencies that can *vary independently* once the total and expected distribution are fixed, since the final observed frequency is determined once all others are known.

The test statistic can be compared against the $\chi^{\text{2}}$ critical values from **page 14** of the data booklet. With $\nu=5$ and taking $\alpha=\text{0.05}$, the critical value of $\chi^{\text{2}}_{\text{5},\text{0.95}}=\text{11.070}$ can be found. The smallest possible observed test statistic is 0, representing data that perfectly matches the model, and for $\nu=\text{5}$ approximately 5\% of test statistics would exceed 11.070 by chance alone. Note that since the test statistic cannot be negative, and can only move further from 0 in the positive direction, the chi-squared tests covered in this course are always *one-tailed*.

Since $\text{3}<\text{11.070}$ the students should *not* reject the null hypothesis at the 5\% level of significance.\newline There is *insufficient evidence* to suggest that the die is not fair.

Note that whilst the $\chi^{\text{2}}$ test statistic is a measure of how well data fits a model, the goodness-of-fit test does not show evidence *for* the model - it only assesses whether the data provides evidence *against* it.

\begin{centering}
\tcbset{width=16cm}
\begin{tcolorbox}[adjusted title=Conditions for valid use of a goodness-of-fit test:, halign title = center,colback=red!5,colframe=red!70!black, halign = center]
\begin{itemize}
...
\textbf{Problem:} A chocolatier says that their boxes are randomly filled with caramel, coffee and strawberry flavoured chocolates in the ratio $\text{2}:\text{2}:\text{1}$. Suspecting that this is not true, a chocolate fan counts the number of chocolates of each flavour in a box and finds 8 are caramel, 13 are coffee and 11 are strawberry. Perform a non-parametric hypothesis test to assess the evidence at the 10\% significance level that the flavours are not distributed as claimed by the chocolatier. 
\tcblower
\textbf{Solution:}
...
\begin{math}
\begin{array}{ll}
      \text{H}_\text{0}: & \text{the chocolates' flavours are distributed in the ratio 2:2:1}\\
      \text{H}_\text{1}: & \text{the chocolates' flavours  are distributed differently}
    \end{array}
\end{math}
...
$\nu=\text{3}-\text{1}-\text{0}=\text{2}$\\

Test statistic: $\chi^{\text{2}}=\frac{(\text{8}-\text{12.8})^\text{2}}{\text{12.8}}+\frac{(\text{13}-\text{12.8})^\text{2}}{\text{12.8}}+\frac{(\text{11}-\text{6.4})^\text{2}}{\text{6.4}}=\text{5.109}$\\

Critical value: $\chi^{\text{2}}=\text{4.605}$
\hspace{6.5cm}
...
\node[above] at (axis cs: 4,0.13)  {$\chi^{\text{2}}_\text{2}$};
\node[below] at (axis cs: 4.6, 0) {4.605};
\node[above] at (axis cs: 7,0.06) {10\%};
\draw[very thick] (axis cs: 5.109,0) -- (axis cs: 5.109,0.25) node[above] {5.109};
\end{axis}
\end{tikzpicture}\\

Since $\text{5.109}>\text{4.605}$, reject $\text{H}_\text{0}$ at the 10\% level of significance. There is sufficient evidence to suggest that the flavours are not distributed in the ratio stated by the chocolatier.

\end{tcolorbox}

Note that the $\chi^{\text{2}}$ distributions displayed in the example solutions are illustrative of their typical characteristic shape, rather than precise graphs of each distribution matching the degrees of freedom.
...
\end{tabular}
\end{center}
Perform a non-parametric test at the 10\% significance level to assess whether there is evidence against the textbook's statement.
...
Perform a $\chi^{\text{2}}$ goodness-of-fit test to determine whether there is evidence at the 1\% significance level to suggest the publisher's claim is not correct. 
...
\begin{math}
\begin{array}{ll}
      \text{H}_\text{0}: & \text{the item types are distributed as claimed}\\
      \text{H}_\text{1}: & \text{they are distributed differently}
    \end{array}
\end{math}
...
Test statistic: $\chi^{\text{2}}=\frac{(\text{33}-\text{30})^\text{2}}{\text{30}}+\frac{(\text{16}-\text{12.5})^\text{2}}{\text{12.5}}+\frac{(\text{1}-\text{7.5})^\text{2}}{\text{7.5}}=\text{6.913}$\\

\vspace{0.3cm}

Critical value: $\chi^{\text{2}}=\text{9.210}$
\hspace{6.5cm}
\begin{tikzpicture}[overlay, 
    declare function={gamma(\z)=
    (2.506628274631*sqrt(1/\z) + 0.20888568*(1/\z)^(1.5) + 0.00870357*(1/\z)^(2.5) - (174.2106599*(1/\z)^(3.5))/25920 - (715.6423511*(1/\z)^(4.5))/1244160)*exp((-ln(1/\z)-1)*\z);},
...
\node[below] at (axis cs: 0,0)  {0};
\node[above] at (axis cs: 4,0.13)  {$\chi^{\text{2}}$};
\node[below] at (axis cs: 9.2, 0) {9.210};
\node[above] at (axis cs: 9.5,0.01) {1\%};
\draw[very thick] (axis cs: 6.913,0) -- (axis cs: 6.913,0.25) node[above] {6.913};
\end{axis}
\end{tikzpicture}\\

\vspace{0.3cm}

Since $\text{6.913}<\text{9.210}$, do not reject $\text{H}_\text{0}$ at the 1\% level of significance. There is insufficient evidence to suggest that the item types are not distributed as stated by the game's publisher.
...
\end{center}
    ...
    \item A coffee shop sells five types of coffee. Historically, the five types of coffee have been sold in the following proportions:
    \begin{center}
        \begin{itemize}
        \item 34\% cappuccinos
        \item 27\% lattes
...
Perform a non-parametric test to assess at the 5\% level of significance the claim that the historical proportions of each type of coffee sold are no longer accurate.
  \item A sample of categorical data is to be collected to assess whether there is evidence to suggest the population is no longer distributed in the 4:3:2:1 ratio for the four categories as previously was recorded. Determine a range of values for the sample size $n$ such that:
  \begin{enumerate}
      \item A chi-squared goodness-of-fit test can be performed with exactly 3 degrees of freedom.
      \item A chi-squared goodness-of-fit test can be performed with exactly 2 degrees of freedom.
  \end{enumerate}
\end{enumerate}
...
The supermarket decides that the number of eggs broken in each box of four follows a binomial distribution, with $p=\text{0.1}$. Perform a hypothesis test to assess this claim and comment on the findings with reference to the assumptions required for a binomial model.
...
\begin{math}
\begin{array}{ll}
      \text{H}_\text{0}: & \text{No. of eggs broken per box follows a } \text{B}(\text{4},\text{0.1}) \text{ distribution.}\\
      \text{H}_\text{1}: & \text{No. of eggs broken per box follows a different distribution.}
    \end{array}
\end{math}\\

\vspace{0.1cm}

Total observations \textbf{144}. Letting random variable $X$ represent the number of eggs broken in a box:\\

\vspace{-0.3cm}

\begin{center}
\begin{tabular}{c c c c}
    $\text{P}(X=\text{0})=\text{0.6561}$ & $\text{P}(X=\text{1})=\text{0.2916}$ & $\text{P}(X=\text{2})=\text{0.0486}$ & $\cdots$\\[1em]
    $\text{0.6561}\times\textbf{144}=\text{94.48}$ & $\text{0.2916}\times\textbf{144}=\text{41.99}$ & $\text{0.0486}\times\textbf{144}=\text{7.00}$ & $\cdots$
\end{tabular}
\end{center}
...
Test statistic: $\chi^{\text{2}}=\frac{(\text{122}-\text{94.48})^\text{2}}{\text{94.48}}+\frac{(\text{10}-\text{41.99})^\text{2}}{\text{41.99}}+\frac{(\text{12}-\text{7.53})^\text{2}}{\text{7.53}}=\text{35.041}$\\

\vspace{0.1cm}

Critical value: $\chi^{\text{2}}=\text{5.991}$
\hspace{6.8cm}
\begin{tikzpicture}[overlay, 
    declare function={gamma(\z)=
    (2.506628274631*sqrt(1/\z) + 0.20888568*(1/\z)^(1.5) + 0.00870357*(1/\z)^(2.5) - (174.2106599*(1/\z)^(3.5))/25920 - (715.6423511*(1/\z)^(4.5))/1244160)*exp((-ln(1/\z)-1)*\z);},
...
\draw[thick,stealth-] (axis cs: 10,0.1) -- (axis cs: 7,0.2) node[above] {35.041}; 
\end{axis}
\end{tikzpicture}\\

Since $\text{35.041}>\text{5.991}$, reject $\text{H}_\text{0}$ at the 5\% level of significance. There is sufficient evidence to suggest that the number of eggs broken per box does not follow a $\text{B}(\text{4},\text{0.1})$ distribution.
...
Note that estimating the parameter $p$ from *the data itself* would require an additional degree of freedom to be subtracted (since $m=\text{1}$). For a binomial distribution, that is *estimating* the probability of success for each trial, $p$, by dividing the total number of successes (broken eggs) by the total number of trials (total eggs). Here:

\[\hat{p}=\dfrac{\text{broken eggs}}{\text{total eggs}}=\dfrac{\text{1}\times \text{10} + \text{3}\times \text{4} + \text{4} \times \text{8}}{\text{4}\times(\text{122}+\text{10}+\text{4}+\text{8})}=\dfrac{\text{54}}{\text{576}}=\text{0.09375}\]

This could be used to check if the number of broken eggs per box fits *any* possible $\text{B}(\text{4},p)$ distribution.
...
\begin{enumerate}
    ...
    \end{enumerate}
    To investigate this, the number of correct answers out of five for each of the 80 students is first set out in a table:
    \begin{center}
    \begin{tabular}{| c | c | c | c | c|c|c| } 
    \hline
...
Just as with a binomial distribution, using a Poisson distribution as a model for a goodness-of-fit test requires the total observations to be split according to calculated probabilities. If sample data is to be tested against a fully specified Poisson distribution (e.g. $\text{Po}(\text{4.5})$) then no model parameter has been estimated using the data ($m=\text{0}$).

Should the only parameter for a Poisson distribution, $\lambda$, be required to be *estimated from the data*, an additional degree of freedom will be *subtracted*, since $m=\text{1}$. This would arise when looking to compare the observed data against *any* Poisson distribution $\text{Po}(\lambda)$.

In the following example, a value for the model parameter $\lambda$ is not provided, and is instead estimated from the data, so here $m=\text{1}$.

\begin{tcolorbox}[title=Example,colback=RoyalBlue!1!,colframe=RoyalBlue]
\textbf{Problem:} It is generally said that the number of goals scored in a game of football follows a Poisson distribution. A football fan decides to see whether the model still seems reasonable, and records the number of goals scored in every game across a weekend in each of the top four tiers of English football:
...
\tcblower
\textbf{Solution:} $\lambda=\frac{\text{total no. of goals}}{\text{total no. of games}}=\frac{\text{115}}{\text{46}}=\text{2.5}$\\

\begin{math}
\begin{array}{ll}
      \text{H}_\text{0}: & \text{No. of goals per game follows a Poisson distribution (with }\lambda=\text{2.5})\\
      \text{H}_\text{1}: & \text{No. of goals per game follows a different distribution.}
    \end{array}
\end{math}\\

Total observations$ =\text{46}$.\\
$\text{P}(X=\text{0})=\text{0.0821}$\\
Expected frequency for 0 goals $=\text{0.0821}\times\text{46}=\text{3.777}$
...
$\nu=\text{6}-\text{1}-\text{1}=\text{4}$\\

Test statistic: $\chi^{\text{2}}=\frac{(\text{4}-\text{3.777})^\text{2}}{\text{3.777}}+...+\frac{(\text{4}-\text{5.004})^\text{2}}{\text{5.004}}=\text{1.186}$\\

Critical value: $\chi^{\text{2}}=\text{13.277}$
\hspace{6cm}
\begin{tikzpicture}[overlay, 
    declare function={gamma(\z)=
    (2.506628274631*sqrt(1/\z) + 0.20888568*(1/\z)^(1.5) + 0.00870357*(1/\z)^(2.5) - (174.2106599*(1/\z)^(3.5))/25920 - (715.6423511*(1/\z)^(4.5))/1244160)*exp((-ln(1/\z)-1)*\z);},
...
\node[below] at (axis cs: 0,0)  {0};
\node[above] at (axis cs: 6,0.1)  {$\chi^{\text{2}}_\text{4}$};
\node[below] at (axis cs: 9.5,0) {13.277};
\node[above] at (axis cs: 12,0.03) {1\%};
\draw[very thick] (axis cs: 1.186,0) -- (axis cs: 1.186,0.25) node[above] {1.186};
\end{axis}
\end{tikzpicture}\\

Since $\text{1.186}<\text{13.277}$, do not reject $\text{H}_\text{0}$ at the 1\% level of significance.\\
...
\begin{enumerate}
    ...
    \end{center}
    \begin{enumerate}
        \item If the number of shooting stars visible in a section of sky in an hour follows the distribution described in the statistics class, calculate the probability of seeing, in any given hour:
        \begin{enumerate}
...
\begin{enumerate}
    ...
    \end{center}
    \begin{enumerate}
        \item Perform a test at the 10\% significance level to assess the evidence that the data fits a binomial distribution where $p=\text{0.15}$.
        \item A coach comments that some of the five penalty takers are clearly better at taking penalties than others. Explain why this would mean that a binomial would be inappropriate.
    \end{enumerate}
