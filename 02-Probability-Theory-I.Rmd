...
The purpose of statistics is to make sense of the world by analysing data. In practice, however, it is only possible to obtain data from a random sample of the full population of interest, introducing uncertainty that must be accounted for. Probability theory provides the mathematical framework used to describe this uncertainty.

People commonly talk about *chance* in everyday language. For example, 'St Mirren are unlikely to win this weekend', or 'It is probably going to rain today'. However, neither comment assigns a numerical value to quantify those chances. In this chapter, probability as a *numerical measure* will be studied.
...
The probability of an event can be estimated by carrying out repeated trials and calculating the relative frequency with which the event occurs. This estimate is often called the *experimental probability* of the event. For example, if a motorist passes a set of traffic lights every morning and wishes to know the probability of having to wait at a red light on any given morning, they could record the proportion of mornings on which they had to wait at a red light over a period of time. If, over the course of 200 mornings, they have to wait at a red light 114 times:
...
When each outcome in the sample space can reasonably be assumed to be \mbox{\textbf{equally likely}}, then the probability of outcomes or events can be determined theoretically, without need to use experimental probability to form an estimate. This is called *theoretical probability*.
...
When considering two events in probability theory, *"and"* means that *both* events are true, while *"or"* means that *at least one* of the events is true (including the possibility that they both occur, where applicable).
...
\begin{enumerate}
    ...
    \end{tabular}
    \end{center}
    Letting $G$ represent the event that the fruit chosen is \textit{good}, and $A$ the event that it is an \textit{apple}, find:
    \begin{multicols}{3}
    \begin{enumerate}
        \item $\text{P}(G)$.
...
It can help to write out sample spaces. Some sample spaces can be described easily using a simple list, but more complicated sample spaces may require a more organised approach, such as listing outcomes systematically or using a grid. Sample spaces are particularly useful when each outcome is equally likely, as this allows probabilities to be calculated directly by counting outcomes.
...
This is because there are *two ways* to obtain one tail.
...
\begin{enumerate}
    ...
    \end{enumerate}
    \item Each morning, a teacher picks at random whether to start the day with tea, coffee or apple juice, and also picks at random which one of muesli, granola, toast or porridge to eat for breakfast. For each of these random choices, each selection is equally likely, and the choice of drink is independent from the choice of food. By first listing all the possible ways the teacher may start their day, determine the probability that on any particular morning the teacher has:
        \begin{enumerate}
            \item Tea and toast.
    ...
    \begin{tikzpicture}[remember picture,overlay]
        ...
        \end{tikzpicture}
    \begin{enumerate}
        \item Tails on at least two consecutive tosses.
    ...
    \item Two six-sided dice are rolled. Find the probability that the product of the numbers obtained is less than 5.
    ...
    \begin{enumerate}
        ...
        \begin{enumerate}
            ...
            \item Neither Alex nor Charlie is chosen for either task.
            \item At least one of Alex and Charlie is chosen to perform a task.
        \end{enumerate}
    \end{enumerate}
\end{enumerate}
...
Note that in the tree diagram above the probability of the card being a diamond does not change depending on whether the roll of the die gives a six or not; this is because the roll of the die and the drawing of the card are **independent**. In the following example the probabilities on the second stage of the tree diagram do change depending on the result of the first stage, since the first and second draws from the bag are **not independent**.
...
\begin{enumerate}
    ...
    \end{enumerate}
    \item A game involves a player randomly selecting one card then a second card from a set of six blue cards and four red cards, without replacement. Find the probability of a player selecting two cards of the same colour.
    \item Suppose that 41\% of the population of Scotland have the blood type \textit{O positive (O+)}. A doctor in Scotland randomly selects two of their patients and asks for them to take part in a blood screening trial, as part of which their blood type will be checked. Calculate the probability that neither of the two patients has blood type \textit{O+}.
    ...
    \begin{enumerate}
        ...
        \item Determine the probability that the two pencils selected are of different types.
        \item Find the probability that neither pencil selected is an HB pencil.
    \end{enumerate}
    \item A motorist passes through three sets of traffic lights on their way to work each day. Assume that the lights operate independently. If the probability of having to stop at a red light for each set of lights is 0.6, 0.2 and 0.3 respectively then find the probability that:
    \begin{enumerate}
...
Venn diagrams are used to show which elements of a set belong to one or more subsets. They can offer an alternative way of representing information about a sample space that might otherwise be shown in a table. For instance, the *contingency table* below shows the twelve dishes from a restaurant's main course menu, broken down by whether or not each is *gluten-free* ($G$) and whether or not each is *vegan* ($V$). Beside it is a Venn diagram representing the same information.
...
\captionof{figure}{\textit{Contingency table}}
  \label{fig:testTable}
  \end{center}
  In statistics, a table used to show frequencies for combinations of categorical variables is often referred to as a \textbf{contingency table}.  
\end{minipage}%
  \centering
\begin{minipage}{.5\textwidth}
  \centering
...
\begin{enumerate}
    ...
    \begin{enumerate}
        \begin{multicols}{2}
            \item $\text{P}(\text{prime})$.
            \item $\text{P}(\text{odd})$.
            \item $\text{P}(\overline{\text{prime}})$.
            \item $\text{P}(\overline{\text{odd}})$.
            \item $\text{P}(\text{prime} \cap \text{odd})$.
            \item $\text{P}(\text{prime} \cup \text{odd})$.
            \item $\text{P}(\overline{\text{prime}} \cap \text{odd})$.
            \item $\text{P}(\text{prime} \cap \overline{\text{odd}})$.
        \end{multicols}
    \end{enumerate}
    \vfill\null
    \end{multicols}
    \vspace{-0.5cm}
    \item 100 pupils are asked whether or not they love cats, and whether or not they love dogs, with the results shown in the table below. One pupil is to be selected at random. Let $C$ represent the event that the selected pupil loves cats, and $D$ represent the event that the selected pupil loves dogs. By first representing this information on a Venn diagram, find: 
    \begin{multicols}{2}
    \begin{enumerate}
    \begin{multicols}{2}
        \item $\text{P}(C\cap D)$.
...
If two events $A$ and $B$ have *no outcomes in common*, they are said to be **mutually exclusive**. This is sometimes visualised on a diagram by two *disjoint* circles that do not intersect. In terms of probability theory, this means that the probability of the event `$A$ *and* $B$' is 0 (impossible).
...
If two events $A$ and $B$ are such that *every outcome in the sample space* lies either in $A$, in $B$, or in both, then they are said to be **exhaustive**. This can be visualised on a Venn diagram by crossing out the area of the sample space outside of the two circles. This means that the probability of the event `$A$ *or* $B$' is 1 (certain).
...
& = \text{1}\hspace{0.2cm}\text{therefore events $F$ and $G$ are exhaustive}
\end{align*}
\end{tcolorbox}
