...
The test statistic is provided in the Data Booklet, and it uses an *estimate of the pooled population proportion*, $\hat{p}$:
...
Whilst $t$-tests are especially valuable when dealing with small sample sizes, they remain the statistically *correct* test to use even when dealing with very large samples. Modern computing power reduces the need to switch to a $z$-test for very large samples where critical values for the $t$-distribution may not have been so easily obtain.
...
\tcblower
\textbf{Solution:}
$\bar{x}_A=203$, $\bar{x}_B=206$, $s_A=3$, $s_B=5$, $n_{\text{A}}=10$, $n_{\text{B}}=15$\\[0.1em]
