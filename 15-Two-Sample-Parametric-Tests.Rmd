...
\textbf{Problem:} To compare the longevity of two brands' light bulbs, a random sample of eight bulbs from \textit{AlwaysBrite} and eight bulbs from \textit{BulbCity} are tested. The sample of AlwaysBrite bulbs had a mean lifespan 1015 hours, with a sample mean of 1059 hours before failure for BulbCity. Assuming that the lifespan for both populations of bulbs is known to be normally distributed, with a standard deviation of 84 hours, use a parametric test to determine whether there is evidence for a difference in longevity between the brands.

\tcblower
\textbf{Solution:}
$\bar{x}_A=1015$, $\bar{x}_B=1059$, $\sigma_{\text{A}}=84$, $\sigma_{\text{B}}=84$, $n_{\text{A}}=8$, $n_{\text{B}}=8$\\[0.1em]
...
\draw[very thick] (axis cs: -1.05,0) -- (axis cs: -1.05,0.44) node[above] {$-1.05$};
\end{axis}
\end{tikzpicture}\\[0.1em]

Critical value: $z=-1.96$\\

Since $-1.05>-1.96$, do not reject $\text{H}_0$ at the 5\% level of significance. There is insufficient evidence to suggest a difference in the mean lifespans of the two brands of light bulbs.
...
Test statistic: $z=\dfrac{\hat{p}_1-\hat{p}_2}{\sqrt{\hat{p} \hat{q} \left( \dfrac{1}{n_1}+ \dfrac{1}{n_2}\right)}}$ \qquad where \qquad $\hat{p}=\dfrac{n_1\hat{p_1}+n_2\hat{p}_2}{n_1+n_2}$
\end{tcolorbox}
\end{centering}
...
\tcblower
\textbf{Solution:}
$\hat{p}_\text{oat}=0.6$, $\hat{p}_\text{wheat}=0.25$, $n_\text{oat}=50$, $n_\text{wheat}=80$\\[0.1em]

\begin{math}
\begin{array}{ll}
      H_0: & p_\text{oat}=p_\text{wheat}\\
      H_1: & p_\text{oat} \ne p_\text{wheat}
    \end{array}
\end{math}
...
\node[below] at (axis cs: -1.96, 0) {$-2.58$};
\node[above] at (axis cs: -2.2,\valueA) {0.5\%};
\draw[very thick,stealth-] (axis cs: 3,0.2) -- (axis cs: 2,0.4) node[above] {$3.99$};
\end{axis}
\end{tikzpicture}\\

Since $3.99>2.58$, reject $\text{H}_0$ at the 1\% level of significance. There is sufficient evidence to suggest that the proportions of men who experience a reduction in triglyceride levels by consuming oat bran or wheat bran daily are not the same. 
...
\tcblower
\textbf{Solution:}
$\bar{x}_\text{A}=203$, $\bar{x}_\text{B}=206$, $s_\text{A}=3$, $s_\text{B}=5$, $n_{\text{A}}=10$, $n_{\text{B}}=15$\\[0.1em]
...
Critical value: $t=-2.069$\hspace{6cm}
\begin{tikzpicture}[overlay]
\begin{axis}[
  no markers, 
  domain=-3:3, 
...
\node[below] at (axis cs: 2.2, 0) {$2.069$};
\node[above] at (axis cs: 2.4,\valueA) {2.5\%};
\draw[very thick] (axis cs: -1.697,0) -- (axis cs: -1.697,0.44) node[above] {$-1.697$};
\end{axis}
\end{tikzpicture}\\[0.5em]

Since $-1.697>-2.069$, do not reject $\text{H}_0$ at the 5\% level of significance. There is insufficient evidence to suggest that there is a difference in the mean volume dispensed by the two machines.
...
\begin{enumerate}
    ...
    \end{tabular}
    \end{center}
    Carry out a hypothesis test to assess for any evidence of a difference in the brands' durability, stating one assumption.
...
Careful attention to how the sample of differences was created is needed for one-tailed hypotheses.
...
\node[below] at (axis cs: 2.2, 0) {$2.571$};
\node[above] at (axis cs: 2.4,\valueA) {2.5\%};
\draw[very thick] (axis cs: 0.84,0) -- (axis cs: 0.84,0.44) node[above] {$0.840$};
\end{axis}
\end{tikzpicture}\\[0.1em]
