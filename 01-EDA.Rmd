...
Knowing what *type* of data has been collected helps determine which statistical methods and diagrams are appropriate. In this course, it is necessary to be able to recognise three distinct types.

*Qualitative* data, also called **categorical** data, is *non-numerical*, such as the colour of a squirrel.

Data that is *numerical* and represents a count or measurement, such as a person's age, is called *quantitative*. Quantitative data can be further classified into one of two types. **Discrete** data consists of values that can be counted or listed, such as the number of siblings someone has. **Continuous** data can take *any* value within a given range, though in practice measured values are often rounded. Examples include the mass of a mouse.

\begin{center}
\begin{tikzpicture}
\node[draw,fill=black!5,rectangle, text centered,rounded corners,minimum height=0.7cm] at (0,-0.4) (data) {What type of data is it?};
\node[draw,fill=black!5,rectangle, text centered,rounded corners,minimum height=0.7cm] at (-5.5,-2) (qualitative) {Qualitative};
\path[draw,-stealth] (data) -- (0,-1) -- (-5.5,-1) -- (qualitative);
...
\node[draw,fill=black!15,rectangle, text centered,rounded corners,minimum height=0.7cm] at (-5.5,-4) (categorical) {\textbf{Categorical}};
...
\node[right] at (-5.5,-3) {\textit{also called}};
\node[above] at (8,-3) {countable values};
\node[below] at (8,-3) {\textit{e.g. a list of values}};
\node[below] at (4,-3) {\textit{e.g. a range of values}};
\node[above] at (4,-3) {measurable values};
\end{tikzpicture}
\end{center}
...
\node[anchor=west] at (8.1,11) {Goals};
\node[anchor=west] at (10,11) {Assists};

\draw (0,10.75) -- (11.5,10.75);

\node at (0.8,10.5) {Midfielder};
\node at (0.8,10) {Defender};
\node at (0.8,9.5) {Forward};
\node at (0.8,9) {Forward};

\node at (3.6,10.5) {Blackburn Rovers};
\node at (3.6,10) {Sheffield United};
\node at (3.6,9.5) {St Mirren};
\node at (3.6,9) {Celtic};
...
\draw[decoration={brace,mirror,raise=2pt},decorate]
  (0,8.7) -- node[below=5pt] {\textbf{Categorical}} (4.7,8.7);

\draw[decoration={brace,mirror,raise=2pt},decorate]
  (5.3,8.7) -- node[below=5pt] {\textbf{Continuous}} (7.2,8.7);

\draw[decoration={brace,mirror,raise=2pt},decorate]
  (8.2,8.7) -- node[below=5pt] {\textbf{Discrete}} (11,8.7);
...
\begin{enumerate}
    \item A survey is carried out on the pets owned by a class. State whether each would result in \textbf{quantitative} or \textbf{qualitative} data.
    \begin{enumerate}
    \begin{multicols}{2}
        \item How many pets each pupil owns.
        \item What kind of species the pet is.
    \end{multicols}
    \end{enumerate}
    ...
    \begin{tabular}{|c|c|c|c|c|c|}
      ...
      \hline
        ...
        A lot & 73.5 & 161 & 47 & 97 & No\\
        Moderate & 73 & 192 & 66 & 26 & Yes\\
      \hline
    \end{tabular}
    \end{center}
    State whether each variable records \textbf{categorical}, \textbf{discrete} or \textbf{continuous} data.
\end{enumerate}
...
As part of any analysis of a dataset, it is common practice to *display* the data in a visual form so that an initial impression can be formed. Typically labelled as \textit{figures}, four key types of plot for the course are shown below, using a sample of 10 sunflower heights recorded by a gardener 8 weeks after germination, measured to the nearest centimetre:
...
\node[text width = 8cm,below] at (8,3.2) {Stem-and-leaf diagrams group data while preserving the original values. Leaves should be in order, and a \textit{key} and the sample size ($n$) included.};
\draw[white] (3.5,-2) rectangle (12.5,4);
\end{tikzpicture}
...
\node[text width = 8cm,below] at (8,3.6) {Histograms display \textit{continuous} data grouped into intervals. Unlike bar charts, the bars are adjacent (no gaps). When interval widths are equal, the height of each bar represents frequency.};
\draw[white] (3.5,-2) rectangle (12.5,5);
\end{tikzpicture}
...
\begin{enumerate}
    ...
    \end{tabular}
    \end{center}
    \item The masses (in kg) of the luggage of 10 passengers on a plane to Barra were recorded, and a five-number summary of the data obtained using \texttt{computer output} from statistical software. Construct a box plot.
    \begin{center}
...
There are *many* ways in which data can be presented, including displays beyond those covered on the previous page. In statistical reports, these are often clearly labelled as **tables** or **figures**, and numbered so they can be more easily referred to. 
...
Not all graphs encountered in real life will be well designed; some graphs may be unclear, accidentally misleading or even intentionally misleading. Being alert to potential flaws in tables and figures and identifying possible improvements is an important skill.

The widespread availability of computing power means that specialist software is often used by statisticians to avoid the need for time-consuming calculations where possible. While this course requires no use of statistical software, examples of \texttt{computer output} will be displayed from which the required information must be extracted.
...
While this textbook aims to enable familiarity with common figures, tables and computer output, the best way to practise obtaining information from them is to *read a wide range of relevant statistical articles and reports*. 
...
In a statistical study, the term **population** refers to the entire set of objects of interest, while a **sample** is a subset chosen from the population. Understanding the difference is fundamental to statistics. 
...
If data were available for an entire *population*, then various **population parameters** could be calculated *exactly*, such as the *population mean*. Since this is rarely possible in practice, *samples* are usually taken from populations instead. Measures calculated from a sample are called **sample statistics**, such as the *sample mean*. 
...
Sample statistics vary depending on the sample chosen and can be viewed as *estimates* of the true values of their corresponding population parameters. The population parameters themselves are treated as fixed, but are usually unknown.

\begin{multicols}{2}

\textit{Statistical inference} is the process of using \textit{sample statistics} to assess claims about \textit{population parameters}. This requires an understanding of the inherent \textit{random variation} that arises when samples are selected from a population. Statistical inference, and \textit{sampling techniques}, will be introduced in later chapters.
...
The purpose of discussing sampling at this point is to caution against making *definitive* statements based solely on *sample statistics*, when the (usually unknown) *population parameters* are of primary interest.
...
\begin{itemize}
    ...
    \item \textbf{Variance} - the \textit{mean of the squared differences} between each value and the mean.
...
A lay person's interpretation of what an 'average' seeks to describe is the *typical* value for a dataset. However, consider the hourly rate of pay of six people working in a store, with one manager and five shop workers:
...
The *lower quartile* (LQ or $\text{Q}_\text{1}$) and the *upper quartile* (UQ or $\text{Q}_\text{3}$) split an ordered dataset into four approximately equal parts, along with the median. From the lowest value to LQ contains approximately 25\% of the data, from LQ to the median contains approximately the next 25\%, and so on.
...
The *interquartile range*, calculated as $\text{IQR}=\text{UQ}-\text{LQ}$, measures the range of the approximate 'middle 50\%' of a dataset, ignoring the remaining lowest and highest quarters. On a box plot (sometimes called a *box-and-whisker* plot), the interquartile range defines the *width of the box*, ignoring the 'whiskers' at either side. The larger a dataset's interquartile range is, the more spread it suggests the data may be.
...
Since in practice a population is rarely available, instead it is common to calculate the **sample mean** ($\bar{x}$), using a sample of data from a population. Given a sample of data $x_\text{1}, x_\text{2}, \dots, x_n$, where $n$ is the sample size, the sample mean can be calculated as, in its most commonly used form:
...
Replicating the same formula using sample data gives a *biased* estimate for variance. This is because the formula requires the population mean $\mu$ be replaced with the *sample mean* $\bar{x}$ as its estimate, which will tend to minimize the sum of square deviations and so tend to lead to an *underestimate* of the true population variance. *Bessel's correction* is used to produce a formula for the **sample variance** ($s^\text{2}$), which is an *unbiased* estimator for the population variance, taking a denominator of $n-\text{1}$. The below two forms are equivalent.
...
\begin{enumerate}
    \item Strawberry tarts are sold every day in a bakery over the summer months. The shop manager monitors the sales and takes a sample of the number of strawberry tarts sold on 10 randomly selected days over one month.
    \begin{center}
    \begin{tabular}{cccccccccc}
        23 & 31 & 45 & 32 & 35 & 19 & 24 & 25 & 30 & 38
    \end{tabular}
    \end{center}
    Calculate the upper and lower fences for this dataset and identify any possible outliers.
...
## Describing the Shapes of Distributions
