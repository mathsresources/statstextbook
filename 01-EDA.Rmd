...
Samples statistics vary depending on the sample chosen, and may be seen as *estimates* of the true value of their respective population parameters, which are considered as fixed values (for a given study, at least). *Statistical inference* is the process us using *sample statistics* to make conclusions about *population parameters*.


\begin{center}
\begin{tikzpicture}[scale=0.8]
\draw[semithick,fill=black!10] (-0.3,2) rectangle (2.7,4);
\node[align=center] at (1.2,4.5) {Population};
\draw[semithick,fill=black!10] (9.5,3) circle (1.1cm);
\node[align=center] at (9.5,4.5) {Sample};
\node[text width=2cm,align=center] at (1.2,3) {Population parameter};
\node[text width = 2cm,align=center] at (9.5,3.1) {Sample statistic};
\draw[very thick,-stealth] (3,3.4)
      arc[start angle=100,end angle=80,radius=14.3969];
\node[black] at (5.5,4) {\textit{Sampling}};
\draw[very thick,stealth-] (3,2.6)
      arc[start angle=-100,end angle=-80,radius=14.396926];
\node at (5.5,2) {\textit{Inference}};
\end{tikzpicture}
\end{center}

\end{multicols}

Both *sampling techniques* and *statistical inference* will be introduced in later chapters. The reason for discussing sampling at this point is to caution against making any *definitive-sounding* statements based on *sample statistics*, when it is typically the (unknown) *population parameters* that are really of interest.
...
\textit{For each, state using the context of the question which population parameter or sample statistic has been obtained, as well as the suitable notation.}
