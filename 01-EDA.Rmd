...
Data that is numerical is called *quantitative*, of which there are two types. **Discrete** data consists of values that can be counted, such as someone's shoe size or the number of siblings they have. **Continuous** data can take *any* value within a range, though in practice values are is often rounded due to measurement precision, such as the mass of a mouse.
...
In a statistical study, the term **population** refers to the entire set of objects of interest, whilst a **sample** is a subset chosen from the population. Recognising the difference is fundamental to statistics. 
...
Sample statistics vary based on the sample chosen, and may be seen as \textit{estimates} of the true value of their respective population parameters, which are seen as fixed values (for a given study, at least). 

\begin{multicols}{2}

\textit{Statistical inference} is the process of using \textit{sample statistics} to assess claims about \textit{population parameters}. This requires learning about the inherent \textit{random chance} involved in selecting a sample from a population. Statistical inference, and \textit{sampling techniques}, will be introduced in later chapters.
...
n & mean & sd & sum & sum of squares & min & LQ & median & UQ & max & range & IQR & mode\\
  \hline
11 & 11.45 & 7.54 & 126 & 2012 & 0 & 2 & 14 & 18 & 20 & 20 & 14 & 18
\end{tabular}
\end{center}

There are two main types of summary statistic: those that measure the \textit{central tendency} of data (or its \textbf{location}); and those that measure the \textit{variability} of data (or its \textbf{spread}). It is common to measure both the location *and* spread.
...
\begin{itemize}
    \item \textbf{Mode} - the most frequently occuring value.
...
The *lower quartile* (LQ or $\text{Q}_\text{1}$) and the *upper quartile* (UQ or $\text{Q}_\text{3}$) split data into quarters, along with the median. From the lowest value to $\text{Q}_\text{1}$ contains approximately 25\% of the data, from $\text{Q}_\text{1}$ to $\text{Q}_\text{2}$ contains approximtely the next 25\%, and so on.
...
Before any analysis of a data set is undertaken, it is common practice to *display* the data in a simple form so that an initial impression of the data can be formed. Some key examples are shown below, using a sample taken by a gardener of 10 of their sunflowers' heights 8 weeks after germination, measured to the nearest centimetre:
