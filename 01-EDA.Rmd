...
\node[above] at (4,-3) {measureable values};
\end{tikzpicture}
\end{center}
...
\begin{enumerate}
    \item A survey is carried out on the pets owned by a class. State the type of data that would be collected.
    \begin{enumerate}
    \begin{multicols}{2}
    ...
    \end{multicols}
    \end{enumerate}
    \item For each of the following, state whether the quantitative data collected would be discrete or continuous.
    \begin{enumerate}
    \begin{multicols}{2}
...
The reason for discussing sampling at this point is to caution against making *definitive* statements based solely on *sample statistics*, when it is typically the (unknown) *population parameters* that are really of interest.
...
\begin{enumerate}
    ...
    \item For a study on lunchtime spending in Scottish schools, the mean lunchtime spend is calculated from surveying every pupil studying as a large secondary school in Edinburgh.
\end{enumerate}
...
There are two main types of summary statistic: those that measure the \textit{central tendency} of data (or its \textbf{location}); and those that measure the \textit{variability} of data (or its \textbf{spread}). It is common to measure both the location *and* spread, since two data sets can have the same average but very different variability.
...
The measure of spread which is usually encountered first in a maths classroom is the *range*, less informative than other measures, as it depends only on the two most extreme values. In practice this measure is seldom used by statisticians since it only uses the two most extreme values from the data. There are three main measures of spread commonly used, two of which are likely to already be familiar: 
...
The *mean* and *standard deviation* are perhaps the most commonly used measures of location and spread, respectively, and both link strongly to the commonly-encountered *bell-shaped curve* (see page 13). The formulae for calculating the sample mean and sample standard deviation from sample data are given below using notation, where the Greek letter $\sum$ (*"Sigma"*) denotes summation (i.e. adding a set of values), $x_i$ represents each value in the sample (for $i=\text{1},\text{2},\dots,n$) and $n$ represents the *number* of values in the sample.

The *sample mean*, $\bar{x}$, is calculated as: \hspace{0.5cm}$\bar{x}=\dfrac{\sum x}{n}$

The *sample standard deviation*, $s$, is calculated as: \hspace{0.5cm}$s=\sqrt{\dfrac{S_{xx}}{n-1}}$\hspace{0.5cm}where: \hspace{0.5cm}$S_{xx}=\sum x^\text{2}-\dfrac{(\sum x)^\text{2}}{n}$

The *sample variance*, $s^\text{2}$, is simply the *square of the standard deviation*.

#### The Median and the Interquartile Range {-}

The median, $\text{Q}_\text{2}$, and interquartile range, $\text{IQR}$, are often chosen to measure location and spread when the data is particularly *skewed* (see page 13), as they are less influenced by *extreme values* than the mean and standard deviation.

The *lower quartile* (LQ or $\text{Q}_\text{1}$) and the *upper quartile* (UQ or $\text{Q}_\text{3}$) split an ordered data set into four equal parts, along with the median. From the lowest value to $\text{Q}_\text{1}$ contains approximately 25\% of the data, from $\text{Q}_\text{1}$ to $\text{Q}_\text{2}$ contains approximately the next 25\%, and so on.
...
\node at (13,0) {22};
\node at (14,0) {26};
\draw[thick] (7.5,-0.5) -- (7.5,0.5) node[above] {$\text{Q}_\text{2}=\text{15}$};
\draw[thick] (4,0) circle (0.5);
\draw[thick] (12,0) circle (0.5);
\node[above] at (4,0.5) {$\text{Q}_\text{1}=\text{8}$};
\node[above] at (12,0.5) {$\text{Q}_\text{3}=\text{22}$};
\draw[stealth-] (0.7,0.3) -- (0.3,0.6) node[above left] {$\text{Min}=\text{4}$};
\draw[stealth-] (14.3,0.3) -- (14.6,0.6) node[above right] {$\text{Max}=\text{26}$};
\end{tikzpicture}
\end{center}

The *interquartile range* is calculated as: \hspace{0.5cm}$\text{IQR}=\text{Q}_\text{3}-\text{Q}_\text{1}$
...
\begin{enumerate}
    ...
    \begin{center}
    \begin{tabular}{ccc}
      $\sum{x}=\text{359}$ & $\sum{x^{\text{2}}}=\text{15119}$ & $n=\text{10}$
    \end{tabular}
    \end{center}
    \item Daily fuel spend ($x$, in £'s) for 15 drivers is summarised below. Calculate the mean and standard deviation.
    \begin{center}
    \begin{tabular}{ccc}
      $\sum{x}=\text{87}$ & $\sum{x^{\text{2}}}=\text{569}$ & $n=\text{15}$
    \end{tabular}
    \end{center}
...
Before analysing a data set, it is common practice to *display* the data in a simple form so that an initial impression can be formed. Some key examples are shown below, using a sample of 10 sunflower heights measured by a gardener 8 weeks after germination, measured to the nearest centimetre:
...
\node at (8,4.5) {\textbf{Dot Plot}};
\node[text width = 8cm,below] at (8,4) {Dot plots display individual data values, stacking repeated values vertically for clarity.};
...
\node at (8,4.5) {\textbf{Stem-and-Leaf Diagram}};
\node[text width = 8cm,below] at (8,4) {Stem-and-leaf diagrams organise data by place value while preserving the original data values. Leaves should be in order, and a key (or \textit{legend}) and the sample size should be included.};
\draw[white] (3.5,-2) rectangle (12.5,5);
\end{tikzpicture}
...
\node at (0.15*53,-0.8) {\text{Height (cm)}};
\node at (8,4.5) {\textbf{Box Plot}};
\node[text width = 8cm,below] at (8,4) {Box plots offer a visual representation of a five-number summary, consisting of the minimum value, the lower quartile $\text{Q}_\text{1}$, the median, the upper quartile $\text{Q}_\text{3}$ and the maximum value.};
\draw[white] (3.5,-2) rectangle (12.5,5);
\end{tikzpicture}
...
\node at (8,4.5) {\textbf{Histogram}};
\node[text width = 8cm,below] at (8,4) {Histograms display continuous data grouped into intervals (bins). Unlike bar charts, the bars are adjacent (no gaps), and the area of each bar represents frequency.};
\draw[white] (3.5,-2) rectangle (12.5,5);
\end{tikzpicture}
...
\begin{align*}
  \text{IQR}&=\text{Q}_\text{3}-\text{Q}_\text{1}=51-38=13\\[1em]
  \text{Lower fence}&=\text{Q}_\text{1}-\text{1}.\text{5}\times \text{IQR}=38-\text{1}.\text{5}\times 13=18.5\\[1em]
  \text{Upper fence}&=\text{Q}_\text{3}+\text{1}.\text{5}\times \text{IQR}=51+\text{1}.\text{5}\times 13=70.5
\end{align*}
...
Summary statistics alone are not sufficient to describe the full complexity of the way data is distributed. It is also important to consider and be able to describe the **shape** of the distribution, accomplished most easily using a histogram. Whilst real-life data rarely follows a precise mathematical model *exactly*, a number of shapes are commonly observed. The following data was generated by *simulating random sampling* from populations with these distributions. 
...
\node[text width = 6cm,align=center] at (3,4) {\textit{uniformly distributed data} ($n=\text{103}$)};
\draw (-0.5,0) -- (6.5,0);
...
\begin{tikzpicture}
\draw[white] (-1,-1) rectangle (7,4.7);
\node[text width = 6cm,align=center] at (3,4) {\textit{normally distributed data (bell-shaped distribution)} ($n=\text{215}$)};
\draw (-0.5,0) -- (6.5,0);    
...
\begin{tikzpicture}
\draw[white] (-1,-1) rectangle (7,4.7);
\node[text width = 6cm,align=center] at (3,4) {\textit{bimodal data} ($n=\text{87}$)};
\draw (-0.5,0) -- (6.5,0);
\draw[fill=black!25] (0,0) rectangle (0.25,0);
\draw[fill=black!25] (0.25,0) rectangle (0.5,0);
...
\begin{tikzpicture}
\draw[white] (-1,-1) rectangle (7,4.7);
\node[text width = 6cm,align=center] at (3,4) {\textit{positively skewed data (and so not symmetrical)} ($n=\text{134}$)};
\draw (-0.5,0) -- (6.5,0);
\draw[fill=black!25] (0,0) rectangle (0.25,0);
\draw[fill=black!25] (0.25,0) rectangle (0.5,0);
\draw[fill=black!25] (0.5,0) rectangle (0.75,0);
...
\begin{tikzpicture}
\draw[white] (-1,-1) rectangle (7,4.7);
\node[text width = 6cm,align=center] at (3,4) {\textit{very hard to infer distributions\\ of data from small samples} ($n=\text{19}$)};
\draw (-0.5,0) -- (6.5,0);
\draw[fill=black!25] (0,0) rectangle (0.25,0);
...
\begin{tikzpicture}
\draw[white] (-1,-1) rectangle (7,4.7);
\node[text width = 6cm,align=center] at (3,4) {\textit{negatively skewed data (and so not symmetrical)} ($n=\text{97}$)};
\draw (-0.5,0) -- (6.5,0);
...
At this *exploratory* stage, it is again important to avoid making and *conclusions* relating to the populations from which the sample data was drawn.
