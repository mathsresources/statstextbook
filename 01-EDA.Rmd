...
This process is called *exploratory data analysis*. It helps us understand the data before applying more formal statistical methods.
...
Before analysing a data set, it is common practice to *display* the data in a simple form so that an initial impression can be formed. Some key \textit{figures} for the course are shown below, using a sample of 10 sunflower heights measured by a gardener 8 weeks after germination, measured to the nearest centimetre:
...
\node[text width = 8cm,below] at (8,3.6) {Histograms display continuous data grouped into intervals. Unlike bar charts, the bars are adjacent (no gaps). When interval widths are equal, the height of each bar represents frequency.};
\draw[white] (3.5,-2) rectangle (12.5,5);
\end{tikzpicture}
...
There are two main types of summary statistic: those that describe the \textit{central tendency} of data (or its \textbf{location}); and those that describe the \textit{variability} of data (or its \textbf{spread}). It is common to measure both the location *and* spread, since two data sets can have the same average but very different variability.
...
\node[right] at (-5,-0.3) {Data set A ($n=\text{11}$):};
\node at (1,-0.3) {4};
\node at (2,-0.3) {6};
\node at (3,-0.3) {6};
\node at (4,-0.3) {7};
...
\node[right] at (-5,-2) {Data set B ($n=\text{10}$):};
\node at (1,-2) {5};
\node at (2,-2) {7};
\node at (3,-2) {7};
\node at (4,-2) {9};
...
The *lower quartile* (LQ or $\text{Q}_\text{1}$) and the *upper quartile* (UQ or $\text{Q}_\text{3}$) split an ordered data set into four approximately equal parts, along with the median. From the lowest value to LQ contains approximately 25\% of the data, from LQ to the median contains approximately the next 25\%, and so on.
...
\node[right] at (-5,0) {Data set A ($n=\text{11}$):};
\node at (1,0) {4};
\node at (2,0) {6};
\node at (3,0) {6};
\node at (4,0) {7};
...
The *interquartile range*, calculated as $\text{IQR}=\text{UQ}-\text{LQ}$, measures the range of the approximate 'middle 50\%' of a data set, ignoring the remaining lowest and highest quarters. On a box plot (sometimes called a *box-and-whisker* plot), the interquartile range defines the *width of the box*, ignoring the 'whiskers' at either side. The larger a data set's interquartile range is, the more spread it suggests the data may be.

\begin{center}
\begin{tikzpicture}
\draw[white] (-5.5,-0.5) rectangle (12,3.4);
\draw (0,0.1) -- (10,0.1);
\node[right] at (-5,2.5) {Data set A:};
...
\begin{enumerate}
    ...
    \end{multicols}
    \end{enumerate}
    \item The frequency tables below represent sample data. Calculate the mean for each data set.
    \begin{enumerate}
    \begin{multicols}{2}
    \item 
    ...
    \end{multicols}
    \end{enumerate}
    \item Sets of sample data are shown below. Using a graphical calculator, or by first putting the data set in order, find the median and interquartile range for each data set.
    \begin{enumerate}
    \begin{multicols}{2}
    \item 
    \begin{tabular}{cccccccc}
...
\begin{enumerate}
    \item[(a)] Show that the data set contains an outlier.
...
\begin{enumerate}
    ...
    \end{center}
    \begin{enumerate}
        \item For the species \textit{iris setosa}, determine whether the data set contains any outliers.
