...
\[\mu=\frac{x_\text{1}+x_\text{2}+\dots +x_N}{N}= \frac{\sum x_i}{N} \quad \text{or, abbreviated} \quad \mu=\frac{\sum x_i}{N}\]
...
\[\sigma^\text{2}=\frac{\sum \left(x - \mu\right)^\text{2}}{N}\]

Replicating the same formula using sample data gives a *biased* estimate for variance. This is because the formula requires the population mean $\mu$ be replaced with the *sample mean* $\bar{x}$ as its estimate, which will tend to minimize the sum of square deviations and so tend to lead to an *underestimate* of the true population variance. *Bessel's correction* is therefore used to produce a formula for the **sample variance** ($s^\text{2}$), which is an *unbiased* estimator for the population variance. The below two forms are equivalent.

\[s^\text{2}=\frac{\sum \left(x - \bar{x}\right)^\text{2}}{n-\text{1}} \quad \text{or} \quad s^\text{2}=\frac{\sum x^\text{2} - \frac{\left(\sum x\right)^\text{2}}{n}}{n-\text{1}}\]
...
\[s=\sqrt{\dfrac{S_{xx}}{n-1}}\quad\text{ where }\quad S_{xx}=\sum x^\text{2}-\dfrac{(\sum x)^\text{2}}{n}\]
...
\[\sum x^\text{2} = \text{282},\; \sum x = \text{36},\; n=\text{5} \qquad \bar{x}=\frac{36}{5}=\text{7.2} \qquad s=\sqrt{\frac{\text{282}-\frac{\text{36}^\text{2}}{\text{5}}}{\text{4}}}=\text{2.387}\text{ (to 3dp)}\]
...
\begin{enumerate}
    \item Summary statistics from sample data are given below. Use appropriate formulae to calculate the mean and standard deviation for each sample.
    \begin{enumerate}
    \begin{multicols}{2}
