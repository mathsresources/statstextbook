...
A random variable is a function that assigns a numerical value (such as a probability) to each outcome of a random experiment, used to *model* the behaviour of random processes. Random variables can be used to describe either *discrete* or *continuous* quantitative variables.

Random variables are notated using *capital letters*, such as $X$ and $Y$, whilst *specific values* that they can take are notated with small letters such as $x$ and $y$. The probability that random variable $X$ takes the value $X$ is notated as:
...
A *probability function* assigns probabilities to each possible value of a discrete random variable, and there are a number of ways of presenting the *probability distribution* created. For example, consider a discrete random variable $X$ that takes the value $3$ with probability $0.2$, the value $4$ with probability $0.5$ and the value $5$ with probability $0.3$. Below, the probability distribution for $X$ has been *tabulated* and *graphed*.
...
\begin{tcolorbox}[title=Example,colback=RoyalBlue!1!,colframe=RoyalBlue]
\textbf{Problem:} A bag containing 4 blue and 6 red cubes. One is drawn at random, followed by another without replacing the first. Letting discrete random variable $X$ represent the number of red cubes drawn, calculate $\text{E}(X)$.
