...
A common aim when working with data is to investigate possible relationships between two numerical variables. For example, suppose a sports scientist wishes to explore the relationship between runners' $\text{VO}_{\text{2}}$ max, which is a measure of aerobic fitness, and their 5km running times. From a sample of eight runners, they may record the values of the *two variables for each individual*, presenting the **bivariate data** obtained in a table as follows:
...
\begin{tabular}{|c|c|c|c|c|c|c|c|c|}
    ...
    \hline
      $\text{VO}_{\text{2}}$ max (ml/kg/min) & 50 & 41 & 54 & 42 & 44 & 48 & 40 & 51\\
      \hline
...
The sports scientist constructs a **scatterplot** to visualise the relationship between them, looking for any apparent patterns or problems. In this example, $\text{VO}_{\text{2}}$ max is treated as the **predictor** variable (plotted on the $x$-axis) and 5km running time is treated as the **response** variable (plotted along the $y$-axis). 
...
\begin{center}
    \begin{tikzpicture}[scale=0.7]
        ...
        \node at (0.5*46,0.25*13.5) {$\text{VO}_{\text{2}}$ max (ml/kg/min)};
        \node at (0.5*34,0.25*27.5) {Time};
        \node at (0.5*34,0.25*25) {(mins)};
        ...
        \draw (0.5*40,0.25*33.62) -- (54*0.5,0.25*21.97);
    \end{tikzpicture}
\end{center}

In general, the predictor variable is chosen as the one which may help explain or predict changes in the response variable, and is typically used to calculate \textit{estimates} of the response variable. Here, this suggests that $\text{VO}_{\text{2}}$ max may be used to estimate 5km running time for other runners whose times are not yet known.

\end{multicols}

Although the sample size is very small, the scatterplot seems to suggest that there is a *negative linear relationship* between a runner's $\text{VO}_{\text{2}}$ max and their 5km running time; an increase in $\text{VO}_{\text{2}}$ max value seems to be associated with a decrease in running time. To illustrate this, a **line of best fit** (or *regression line*) has been added to the scatterplot.
...
When deciding whether a linear relationship exists between two numerical variables, a scatterplot should always be drawn before any analysis is undertaken using the bivariate data collected.  This is the first step in determining the existence and strength of any  **linear relationship** (also called **linear correlation**) between the variables. Whilst other kinds of relationship may exist between two numerical variables, in this course the focus is on the existence and strength of *linear* relationships. Consider the following:
...
\textbf{Figure 1} shows a \textbf{strong positive linear relationship} between $x$ and $y$, where both variables tend to increase together.
...
\textbf{Figure 2} shows a \textbf{weak negative linear relationship} between $x$ and $y$, where, as one variable increases, the other tends to decrease. 
...
In \textbf{Figure 3}, as one variable increases, there appears to be no clear pattern as to how the other variable behaves. There appears to be \textbf{no linear relationship} between $x$ and $y$.
...
\textbf{Figure 4} is also an example of \textbf{no linear relationship} between $x$ and $y$, although there appears to be some non-linear (possibly quadratic) relationship between the variables.
...
Note that **correlation does not imply causation**. A linear relationship can exist between variables without implying that one variable causes the other; there may be other variables influencing both. An apparent relationship may also arise by chance. Such a relationship may be described as a *spurious correlation*.

\begin{tcolorbox}[title=Example,colback=RoyalBlue!1!,colframe=RoyalBlue]
\textbf{Problem:} A French class takes two tests: \textit{listening} and \textit{reading}. The results for the nine pupils are shown in the table and scatterplot below. Comment on the scatterplot.
...
\begin{tabular}{|c|c|c|c|c|c|c|c|c|c|}
    \hline
    Listening & 4 & 6 & 9 & 8 & 12 & 13 & 13 & 16 & 18 \\
    \hline
    Reading & 7 & 5 & 12 & 10 & 14 & 13 & 16 & 15 & 19 \\
    \hline
\end{tabular}
...
\draw[thin,black!20,step=2] (0,0) grid (20,20);
...
\node at (10,-6) {listening};
\node[rotate=90] at (-6,10) {reading};
\draw[fill=black] (4,7) circle (0.2);
\draw[fill=black] (6,5) circle (0.2);
\draw[fill=black] (9,12) circle (0.2);
\draw[fill=black] (8,10) circle (0.2);
\draw[fill=black] (12,14) circle (0.2);
\draw[fill=black] (13,13) circle (0.2);
\draw[fill=black] (13,16) circle (0.2);
\draw[fill=black] (16,15) circle (0.2);
\draw[fill=black] (18,19) circle (0.2);
\end{tikzpicture}
\end{center} 
...
\tcblower
\textbf{Solution:} The scatterplot appears to show a strong positive linear relationship between pupils' listening and reading scores.
...
\begin{enumerate}
    \item For each, describe the relationship shown in the scatterplot.
    \begin{enumerate}[rightmargin=20pt]
    \begin{multicols}{2}
        \item A diner in the USA collects data on the \textit{bill amount} and \textit{tip left} by its guests.
\begin{center}
\begin{tikzpicture}[scale=0.8]
...
\draw (-0.1,0) -- (-0.1,4);
\draw (0,-0.1) -- (5,-0.1);
\node[rotate=90,above] at (-1,2) {Sale price (£000s)};
...
\vfill\null
    \end{multicols}
    \vspace{-1cm}
    Comment on any relationship shown in each scatterplot.
...
\Sigma x&=\text{75.13}\\
\Sigma y&=\text{2919}\\
\Sigma x^\text{2}&=\text{272.1}\\
\Sigma y^\text{2}&=\text{416279}\\
\Sigma xy&=\text{9583.94}\\
n&=\text{22}
\end{align*}
\end{minipage}
};
...
\end{center} 
\begin{enumerate}
    \item Using the scatterplot, describe the relationship between song length and tempo.
    \item Explain why this study may not support a conclusion that longer rock songs tend to have a higher tempo than slower rock songs.
...
\begin{center}
\tcbset{width=8cm}
\begin{tcolorbox}[adjusted title=Pearson's Correlation Coefficient, halign title = center,colback=red!5,colframe=red!70!black, halign = center]
...
\begin{align*}
S_{xx}&=\Sigma(x-\bar{x})^\text{2}=\Sigma x^\text{2}-\frac{(\Sigma x)^\text{2}}{n}\\
S_{yy}&=\Sigma(y-\bar{y})^\text{2}=\Sigma y^\text{2}-\frac{(\Sigma y)^\text{2}}{n}\\
...
For all sets of data, $-\text{1}\leqslant r\leqslant \text{1}$. A correlation coefficient of **zero** indicates there is **no linear relationship** between the two variables, whilst any non-zero correlation coefficient suggests the presence of a possible linear relationship. The figures below show scatterplots of sample data along with their sample correlation coefficients $r$ and an interpretation. 
...
As the correlation coefficient, r, is calculated for a *sample*, it is a **sample statistic**. It gives an estimate for the *population correlation coefficient*, $\rho$ (the Greek letter, *"rho"*). For this reason, a non-zero value of $r$ calculated from a sample only *suggests* a linear relationship may exist in the population.
...
\end{tikzpicture}
\end{center}
This \textbf{sample} has resulted in a value for $r$ which approximates well the true \textcolor{black!70}{\textbf{population}} correlation coefficient, $\rho$.
...
\end{tikzpicture}
\end{center}
This \textbf{sample} has resulted in a value for $r$ which is far from the true \textcolor{black!70}{\textbf{population}} correlation coefficient, $\rho$.
...
\begin{tcolorbox}[title=Example,colback=RoyalBlue!1!,colframe=RoyalBlue]
\textbf{Problem:} A French class takes two tests: \textit{listening} and \textit{reading}. The results for the nine pupils are shown in the table below. Calculate the correlation coefficient and interpret its value.
...
\tcblower
\textbf{Solution:} $\sum x^2=1259$, $\sum x=99$, $\sum y^2 = 1525$, $\sum y=111$, $\sum xy =1373$, $n=9$
...
\begin{flalign*}
S_{xx}&=\Sigma x^\text{2}-\frac{(\Sigma x)^\text{2}}{n}\\
&=\text{1259}-\frac{\text{99}^\text{2}}{\text{9}}\\
&=\text{170}
\end{flalign*}
\vfill
\null
\columnbreak
\begin{flalign*}
S_{yy}&=\Sigma y^\text{2}-\frac{(\Sigma y)^\text{2}}{n}\\
&=\text{1525}-\frac{\text{111}^\text{2}}{\text{9}}\\
&=\text{156}
\end{flalign*}
\vfill
\null
...
&=\text{1373}-\frac{\text{99}\times\text{111}}{\text{9}}\\
&=\text{152}
\end{flalign*}
\vfill
\null
...
$r=\dfrac{S_{xy}}{\sqrt{S_{xx}S_{yy}}}=\dfrac{\text{152}}{\sqrt{\text{170}\times\text{156}}}=\text{0.933}$\\[0.5em]

This suggests a strong positive linear correlation between listening test scores and reading test scores.
...
\begin{enumerate}
    \item A biologist records the tail length ($x$) and height ($y$) for a sample of 8 red squirrels. Summary statistics are shown below. Calculate the correlation coefficient and interpret its value.
\begin{align*}
\Sigma x&=\text{143.4} \quad &\Sigma y&=\text{163.3} \quad &\Sigma xy&=\text{2978.5}\\[0.3em]
\Sigma x^\text{2}&=\text{2618.3} &\Sigma y^\text{2}&=\text{3391.8} &n&=\text{8}
\end{align*}
    \item For each dataset, calculate the correlation coefficient and interpret its value.
    \vspace{-0.3cm}
    \begin{enumerate}
    \begin{multicols}{2}
        \item For 20 marathon runners, their age ($x$, in years) and finish time ($y$, in hours) is recorded.
\begin{center}
\vspace{-1cm}
\begin{tikzpicture}[scale=0.8]
...
\node[below] at (2,3.1) {Age (years)};
\node[above,rotate=90] at (-0.1,5) {Time (hours)};
\node[text width = 6cm] at (5.5,5) {
\begin{minipage}{\textwidth}
\begin{align*}
\Sigma x&=\text{785.9}\\
\Sigma y&=\text{96}\\
\Sigma x^\text{2}&=\text{34481}\\
\Sigma y^\text{2}&=\text{472.6}\\
\Sigma xy&=\text{3888.7}
\end{align*}
\end{minipage}
};
\end{tikzpicture}
\end{center}
    \item The age ($x$ years) and selling\\ price ($y$ £000s) of 8 used cars is recorded.
\begin{center}
\vspace{-1cm}
\begin{tikzpicture}[scale=0.8]
...
\node[below] at (2,0.6) {Age (years)};
\node[above,rotate=90] at (-0.1,2.5) {Price (£000s)};
\node[text width = 6cm] at (5.5,2.5) {
\begin{minipage}{\textwidth}
\begin{align*}
\Sigma x&=\text{49.63}\\
\Sigma y&=\text{94.48}\\
\Sigma x^\text{2}&=\text{333.1}\\
\Sigma y^\text{2}&=\text{1208.0}\\
\Sigma xy&=\text{538.87}
\end{align*}
\end{minipage}
};
...
\item Students' average homework mark and their test score are recorded. Calculate the correlation coefficient.
    ...
    \begin{center}
        ...
        \end{tabular}
    \end{center}
    \item A \textit{sepal} is part of a flower. The lengths ($x$, in cm) and widths ($y$, in cm) are measured for a random sample of 100 \textit{iris} flowers, of two different varieties. A scatterplot is plotted, and a correlation coefficient of $-\text{0.206}$ is obtained. The researcher concludes that this proves that there is a weak negative linear relationship.
    \vspace{-0.3cm}
\begin{center}
\begin{tikzpicture}[scale=0.8]
...
A $y$ *on* $x$ regression model is one constructed with the aim of producing a regression line which can be used to predict unknown values of $y$ based on known values of $x$. For a sample of bivariate data $(x_\text{1},y_\text{1}), (x_\text{2},y_\text{2}), ...  ,(x_n,y_n)$, this *regression line* has an equation of the form $\hat{y}=a + b x$, where: 

\begin{figure}[H]
  \centering
\begin{minipage}[c]{.54\textwidth}

$\hat{y}$ (``y hat'') is the \textit{predicted value} of $y$ for a given $x$.\\

Sample statistic $a$ represents the line's \textbf{intercept}, and is an estimate for the true \textit{intercept parameter} $\alpha$ (\textit{``alpha''}).\\

Sample statistic $b$ represents the line's \textbf{slope}, and is an estimate for the true \textit{slope parameter} $\beta$ (\textit{``beta''}). 
...
\begin{center}
    \begin{tikzpicture}[scale=1]
        \draw[semithick,-stealth] (0,0) -- (6,0) node[right] {$x$};
        \draw[semithick,-stealth] (0,0) -- (0,3) node[above] {$y$};
        \node[below] at (3,0) {\textit{predictor}};
        \node[above,rotate=90] at (0,1.5) {\textit{response}};
        \draw[fill] (1,1.105) circle (0.04) node[below] {$(x_\text{1},y_\text{1})$};
        \draw[fill] (2,2.17) circle (0.04) node[above] {$(x_\text{2},y_\text{2})$};
        \draw[fill] (3,1.10) circle (0.04) node[below] {$(x_\text{3},y_\text{3})$};
        \draw[fill] (4,1.43) circle (0.04) node[right] {$(x_\text{4},y_\text{4})$};
        \draw[fill] (5,2.47) circle (0.04) node[above] {$(x_\text{5},y_\text{5})$};
        \draw[thick] (0.7,0.7*0.21+1.016) -- (5.1,5.1*0.21+1.016) node[right] {$\hat{y}=a + b x$};
    \end{tikzpicture}
\end{center}
...
\begin{center}
    \begin{tikzpicture}
        ...
        \node[below] at (3,0) {\textit{predictor}};
        \node[above,rotate=90] at (0,1.5) {\textit{response}};
        \draw[dashed] (1,1.105) -- (1,1.226);
        \draw[fill] (1,1.105) circle (0.04);
        \draw[dashed] (2,2.17) -- (2,1.436);
        \draw[fill] (2,2.17) circle (0.04);
        \draw[dashed] (3,1.10) -- (3,1.646);
        \draw[fill] (3,1.10) circle (0.04);
        \draw[dashed] (4,1.43) -- (4,1.856);
        \draw[fill] (4,1.43) circle (0.04);
        \draw[dashed] (5,2.47) -- (5,2.066);
        \draw[fill] (5,2.47) circle (0.04);
        \draw[thick] (0.7,0.7*0.21+1.016) -- (5.6,5.6*0.21+1.016);
    \end{tikzpicture}
\end{center}
...
\begin{center}
    \begin{tikzpicture}
        ...
        \node[below] at (3,0) {\textit{predictor}};
        \node[above,rotate=90] at (0,1.5) {\textit{response}};
        \fill[black!20] (1,1.105) rectangle (1.121,1.226);
        \draw[dashed] (1,1.105) -- (1,1.226);
        \draw[fill] (1,1.105) circle (0.04);
        \fill[black!20] (2,2.17) rectangle (2.734,1.436);
        \draw[dashed] (2,2.17) -- (2,1.436);
        \draw[fill] (2,2.17) circle (0.04);
        \fill[black!20] (3,1.105) rectangle (3.546,1.646);
        \draw[dashed] (3,1.10) -- (3,1.646);
        \draw[fill] (3,1.10) circle (0.04);
        \fill[black!20] (4,1.43) rectangle (4.426,1.856);
        \draw[dashed] (4,1.43) -- (4,1.856);
        \draw[fill] (4,1.43) circle (0.04);
        \fill[black!20] (5,2.47) rectangle (5.404,2.066);
        \draw[dashed] (5,2.47) -- (5,2.066);
        \draw[fill] (5,2.47) circle (0.04);
        \draw[thick] (0.7,0.7*0.21+1.016) -- (5.6,5.6*0.21+1.016);
    \end{tikzpicture}
...
\begin{center}
\tcbset{width=10cm}
\begin{tcolorbox}[adjusted title=Estimation of Slope and Intercept Parameters, halign title = center,colback=red!5,colframe=red!70!black, halign = center]
\vspace{-0.4cm}
$$\hat{\beta}=b=\dfrac{S_{xy}}{S_{xx}} \qquad \text{and} \qquad \hat{\alpha}=a=\bar{y}-b\bar{x}$$
\end{tcolorbox}
\end{center}

Note that the calculation for $a$ implies that the regression line always passes through the point $\left(\bar{x},\bar{y}\right)$.

Once values for $a$ and $b$ have been calculated, and so the least squares regression line of $y$ on $x$ obtained as $\hat{y}=a+bx$, unknown values of $y$ may be estimated for given values of $x$. This equation (and the line it defines) is *not* suitable for predicting $x$ from $y$, because the least-squares regression line of $y$ on $x$ minimises vertical deviations, not horizontal deviations.

\begin{vwcol}[widths={0.65,0.35},
 sep=.8cm,rule=0pt]
Whether a linear relationship will continue to hold for $x$ values lower or higher than any observed within the range of the data can be unpredictable, and should not be assumed.\\[0.3em]\textbf{Extrapolation}, prediction outwith the range of the data, should be seen as potentially unreliable.\\[0.3em]\textbf{Interpolation}, prediction within the range of the observed data, is generally more reliable, particularly when a strong linear relationship is present.
...
\begin{tcolorbox}[title=Example,colback=RoyalBlue!1!,colframe=RoyalBlue]
\textbf{Problem:} A French class takes two tests: \textit{listening} and \textit{reading}. The results for the nine pupils are shown in the scatterplot below, along with some summary statistics. Comment on the scatterplot.
...
\begin{center}
    \begin{tabular}{lll}
        $\sum x=99$ & $\sum x^2=1259$ & $\sum y=111$\\[0.5em]
        $\sum y^2=1525$ & $\sum xy=1373$ & $n=9$
    \end{tabular}
\end{center}
...
\begin{enumerate}
        \item[(a)] Calculate the equation of the least squares regression line of $y$ on $x$.
        \item[(b)] Calculate a predicted mark for the reading test for a pupil who missed it, having scored 7 in the listening test.
        \item[(c)] Comment on the reliability of this prediction.
\end{enumerate}
...
\node at (10,-4) {listening};
\node[rotate=90] at (-4,10) {reading};
\draw[fill=black] (4,7) circle (0.2);
\draw[fill=black] (6,5) circle (0.2);
\draw[fill=black] (9,12) circle (0.2);
\draw[fill=black] (8,10) circle (0.2);
...
\begin{enumerate}
    \item[(a)]\vspace{-0.3cm} \begin{flalign*}
S_{xx}&=\Sigma x^\text{2}-\frac{(\Sigma x)^\text{2}}{n}=\text{1259}-\frac{\text{99}^\text{2}}{\text{9}}=\text{170}&&\\[1em]
S_{xy}&=\Sigma xy-\frac{\Sigma x\Sigma y}{n}=\text{1373}-\frac{\text{99}\times \text{111}}{\text{9}}=\text{152}\\[1em]
b&=\dfrac{S_{xy}}{S_{xx}}=\dfrac{\text{152}}{\text{170}}=\text{0.894}\\[1em]
a&=\bar{y}-b \bar{x}=\dfrac{\text{111}}{\text{9}}-\text{0.894}\times \dfrac{\text{99}}{\text{9}}=\text{2.499}\\[1em]
\hat{y}&=\text{2.499}+\text{0.894}x
\end{flalign*}
    \item[(b)] $\hat{y}=\text{2.499} + \text{0.894}\times \text{7}=\text{8.757}$
    \item[(c)] This is an example of interpolation as the value of $x$ used is within the range of values of the other students, and consequently it can be considered a reliable prediction, especially given the strong linear relationship suggested by the scatterplot.  
\end{enumerate}

\end{tcolorbox}

Another way of writing the equation found above is: $\text{reading}=\text{2.499}+\text{0.894}\times\text{ listening}$. 

The estimated slope of $b=\text{0.894}$ suggests that each increase of 1 mark in the listening test is associated with an increase of 0.894 marks in the reading test. The estimated intercept of $a=\text{2.499}$ suggests that for a *listening score of zero*, a predicted reading score should be 2.499. Whilst an interpretation of the slope may often be useful, for many contexts an interpretation for the intercept may be less useful, since an $x$ value of zero will not always be realistic or even possible.

Scatterplots should always be constructed and closely scrutinised before further analysing bivariate data. By inspecting a scatterplot, it may become apparent, for instance, that the dataset is separated into *distinct groups*, which ought to be considered separately. *Outliers* can have a large influence on both the correlation coefficient and the regression line, so they should always be (visually) identified and considered, removing them only if doing so can be justified.
...
\item 26 randomly selected trees in a forest have their diameter at a set distance from the ground measured, in inches, and the height of each tree also measured, in feet. The equation of the least squares regression line for $y$ on $x$ is obtained, and shown on the scatterplot below.
\vspace{-0.2cm}
    \begin{center}
    \begin{tikzpicture}
        \draw (0,0) -- (6.2,0);
        ...
        \node at (3.1,-0.8) {Tree diameter (inches)};
        \draw (0,0) -- (0,3.2);
...
\node[right] at (9,2.9) {$\sum x=\text{74.76}$};
\node[right] at (9,2.3) {$\sum x^\text{2}=\text{253.76}$};
\node[right] at (9,1.7) {$\sum y=\text{1408.3}$};
\node[right] at (9,1.1) {$\sum y^\text{2}=\text{77277}$};
\node[right] at (9,0.5) {$\sum xy=\text{4233.1}$};
\node[right] at (9,-0.1) {$n=\text{26}$};
    \end{tikzpicture}
\end{center}
    \vspace{-0.2cm}
    \begin{enumerate}
        \item Determine the equation of the least squares regression line of $y$ on $x$.
        \item Calculate an estimate for the height of a tree with a diameter of 3.9 inches.
    \end{enumerate}
...
\draw[-stealth] (0,0.5) -- (7.3,0.5);
\draw[-stealth] (0,0.5) -- (0,3.7);
\node at (3.5,-0.3) {Time (hours)};
\node at (-1.8  ,2.5) {Distance};
\node at (-2,2) {(miles)};
\node[right] at (9,3.6) {Summary statistics:};
\node[right] at (9,3) {$\sum x=\text{44.5}$};
\node[right] at (9,2.4) {$\sum x^\text{2}=\text{191.21}$};
\node[right] at (9,1.8) {$\sum y=\text{1098}$};
\node[right] at (9,1.2) {$\sum y^\text{2}=\text{116768}$};
\node[right] at (9,0.6) {$\sum xy=\text{4179.2}$};
\node[right] at (9,0) {$n=\text{11}$};
\end{tikzpicture}
\end{center}  
...
\item \textit{Electrolyte powders} are often used to help improve hydration during exercise. A keen cyclist records the time ($y$ minutes) it takes them to complete a familiar route during a series of rides over the summer, randomly choosing the mass ($x$ grams) of electrolyte powder to add to their water bottle each time. 
    ...
    \begin{center}
        \begin{tabular}{cccccc}
            $S_{xx}$ & $S_{yy}$ & $S_{xy}$ & $n$ & $\bar{x}$ & $\bar{y}$\\[0.4em]
            665 &  567.2 & -346.4 & 20 & 10.5 & 57.65
        \end{tabular}
    \end{center}
...
\draw (0,0) -- (0,5);
\node[rotate=90] at (-3,2.5) {Vertical jump height (cm)};
...
\draw[fill=black] (4*0,0.600000000000001) ellipse (0.12 and 0.06);
\draw[fill=black] (4*0.350000000000001,2.2) ellipse (0.12 and 0.06);
\draw[fill=black] (4*0.5,1.8) ellipse (0.12 and 0.06);
\draw[fill=black] (4*1.5,2.6) ellipse (0.12 and 0.06);
\draw[fill=black] (4*3,3) ellipse (0.12 and 0.06);
\draw[fill=black] (4*2.6,2.4) ellipse (0.12 and 0.06);
\draw[fill=black] (4*2.4,2) ellipse (0.12 and 0.06);
\draw[fill=black] (4*1.9,2.2) ellipse (0.12 and 0.06);
\draw[fill=black] (4*2,2.1) ellipse (0.12 and 0.06);
\draw[fill=black] (4*2.75,4.4) ellipse (0.12 and 0.06);
\draw[fill=black] (4*2.5,3.7) ellipse (0.12 and 0.06);
\draw[fill=black] (4*0.75,1.6) ellipse (0.12 and 0.06);
\draw[fill=black] (4*2,2.2) ellipse (0.12 and 0.06);
\draw[fill=black] (4*1,1.5) ellipse (0.12 and 0.06);
\draw[fill=black] (4*2.9,3.6) ellipse (0.12 and 0.06);
\draw[fill=black] (4*2.1,2.5) ellipse (0.12 and 0.06);
\draw[fill=black] (4*2.25,2.6) ellipse (0.12 and 0.06);
\draw[fill=black] (4*2.9,3.2) ellipse (0.12 and 0.06);
\draw[fill=black] (4*1.25,1.8) ellipse (0.12 and 0.06);
\draw[fill=black] (4*1.1,1.5) ellipse (0.12 and 0.06);
\draw[fill=black] (4*3.5,3.4) ellipse (0.12 and 0.06);
\draw[fill=black] (4*2.4,2.6) ellipse (0.12 and 0.06);
\draw[fill=black] (4*3,3.1) ellipse (0.12 and 0.06);
\draw[fill=black] (4*2.25,2.6) ellipse (0.12 and 0.06);
\draw[fill=black] (4*3.5,3.7) ellipse (0.12 and 0.06);
\draw[fill=black] (4*1.25,1.6) ellipse (0.12 and 0.06);
\draw[fill=black] (4*2.25,2.5) ellipse (0.12 and 0.06);
\draw[fill=black] (4*2.5,2.8) ellipse (0.12 and 0.06);
\draw[fill=black] (4*3.25,3.1) ellipse (0.12 and 0.06);
...
\begin{tabular}{c}
  \phantom{-}\\[0.2em]
  $\Sigma x=\text{4673}$\\[0.6em]
  $\Sigma x^\text{2}=\text{763101}$\\[0.6em]
  $\Sigma xy=\text{308020}$\\[0.6em]
  $\Sigma y=\text{1889}$\\[0.6em]
  $\Sigma y^\text{2}=\text{124945}$\\[0.6em]
  $n=\text{29}$
\end{tabular}
\end{center}
...
\vspace{-0.8cm}
    ...
    \begin{enumerate}
        \item[(a)] Use the scatterplot to comment on the relationship between vertical jump height and back squat weight.
        \item[(b)] The coach wishes to use the data to predict vertical jump height.
        \begin{enumerate}
        ...
        \end{enumerate}
        \item[(c) ] Based on the correlation coefficient the coach advises that increasing their back squat weight will increase their vertical jump height. Give a reason why this advice is not necessarily correct.
    \end{enumerate}
    \item A sample of bivariate data collected by a researcher is shown in the table below.
    \begin{center}
        \begin{tabular}{|c|c|c|c|c|c|c|c|c|}
            \hline
    ...
    \item A linear relationship is observed on a scatterplot showing, for a sample of pupils in a statistics class, the amount of time spent reviewing notes and the mark in an assessment. A least squares regression line is obtained with equation.
\end{enumerate}
...
\draw[semithick] (2.0,15.9) -- (5.2,15.9);
\node[rotate=90] at (0.0,0.1*180) {Heart rate (bpm)};
...
\begin{enumerate}
    \item[] However, after considering the scatterplot for the data, it was realised that one piece of data has been misrecorded, and this volunteer’s data (spotted as a possible outlier) was removed from the dataset. 
    \begin{enumerate}
        ...
        \[\Sigma x=\text{518},  \Sigma y=\text{2687},  \Sigma x^\text{2}=\text{19196},  \Sigma y^\text{2}=\text{482691},  \Sigma xy=\text{91534}\]
        \item[(c) ] Use this new equation to obtain a prediction for the peak heart rate of a 42 year old volunteer. 
    \end{enumerate}
\end{enumerate}
...
\begin{enumerate}[resume]
    \item James Forbes was a Scottish physicist who, amongst other things, was interested in finding a way to estimate altitude from measurements of the boiling temperature of water. Some of his observations on boiling point ($y$, in $^\circ\mathrm{F}$) and barometric pressure ($x$, in inches of mercury) are summarised below and shown in the scatterplot.
    \vspace{-0.9cm}
    \begin{center}
    \begin{tikzpicture}[scale=0.8]
        \node[right] at (8,3.4) {Summary data for sample:};
        \node[right] at (8,2.4) {$S_{xx}=\text{145.94}$};
        \node[right] at (8,1.7) {$S_{yy}=\text{530.78}$};
        \node[right] at (8,1) {$S_{xy}=\text{277.54}$};
        \node[right] at (8,0.3) {$\sum x=\text{426}$};
        \node[right] at (8,-0.4) {$\sum y = \text{3450.2}$};
        \node[right] at (9,-1.1) {$n=\text{17}$};
...
\end{center}
\vspace{-0.9cm}
The least squares regression line of $\hat{y}=\text{155.29}+\text{1.9018}x$ was fitted. Show the calculations required to obtain the values of the sample intercept and sample slope parameter.
\end{enumerate}
...
\begin{enumerate}
    ...
    \begin{enumerate}
        ...
        \item[(b) ] Calculate the product moment correlation coefficient for this dataset. 
