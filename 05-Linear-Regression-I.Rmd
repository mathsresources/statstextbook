...
\node[rotate=90] at (-1.7,1.75) {Practical};
\node at (2,-1.7) {Theoretical};
\foreach \y in {6,8,...,20}
...
\node at (9.5,3.5) {$\Sigma x=\text{132},$\quad$\Sigma y=\text{147},$};
\node at (9.5,2.9) {$\Sigma x^\text{2} =\text{1742},$\quad$\Sigma y^\text{2} =\text{2057},$\quad$\Sigma xy=\text{1872}$};
...
\node[anchor=west] at (5.5,0.7) {(b) Calculate a predicted mark for the practical test};
\node[anchor=west] at (6.1,0.2) {for a pupil who missed it, having scored 8 in the};
\node[anchor=west] at (6.1,-0.3) {theoretical test.};
...
S_{xx}&=\Sigma x^\text{2}-\frac{(\Sigma x)^\text{2}}{n}=\text{1742}-\frac{\text{132}^\text{2}}{\text{12}}=\text{290}&&\\[1em]
S_{xy}&=\Sigma xy-\frac{\Sigma x\Sigma y}{n}=\text{1872}-\frac{\text{132}\times \text{147}}{\text{12}}=\text{255}\\[1em]
b&=\dfrac{S_{xy}}{S_{xx}}=\dfrac{\text{255}}{\text{290}}=\text{0.879}\\[1em]
a&=\bar{y}-b \bar{x}=\dfrac{\text{147}}{\text{12}}-\text{0.879}\times \dfrac{\text{132}}{\text{12}}=\text{2.578}\\[1em]
\hat{y}&=\text{2.578}+\text{0.879}x
\end{flalign*}
    \item[(b)] $\hat{y}=\text{2.578} + \text{0.879}\times \text{8}=\text{9.61}$
...
Another way of writing the equation found above is: $\text{practical}=\text{2.578}+\text{0.879}\times\text{ theoretical}$. This suggests that each increase of 1 mark in the theory test is associated with an increase of 0.879 marks in the practical.

In some contexts, the regression line may be written informally as $y=a+bx$, but using $\hat{y}=a+bx$ makes clear that the line gives predicted values rather than observed values.
...
\begin{enumerate}[resume]
    ...
    \begin{tikzpicture}[scale=0.8]
        ...
        \node[right] at (7,1.9) {$S_{xx}=\text{145.94}$};
        \node[right] at (7,1.2) {$S_{yy}=\text{530.78}$};
        \node[right] at (7,0.5) {$S_{xy}=\text{277.54}$};
        \node[right] at (7,-0.2) {$n=\text{17}$};
