...
Note that **correlation doesn't imply causation**. A linear relationship can exist between variables without meaning that one causes the other; there may be other variables influencing both. Seemingly linear relationships can also appear by chance - this is referred to as *spurious correlation*.
...
\begin{center}
    \begin{tikzpicture}[scale=1]
        ...
        \draw[thick] (0.7,0.7*0.21+1.016) -- (5.6,5.6*0.21+1.016) node[right] {$\hat{Y}_i=a + b x_i$};
        \node[right] at (8.7,3) {$\hat{Y}_i$ (\textit{`Y hat'}) is the \textit{best estimate} of $y_i$ given $x_i$};
        \node[right] at (8.7,2) {$a$ represents the line's \textbf{intercept}, and is an estimate for the true \textit{intercept parameter} $\alpha$ (\textit{``alpha''})};
        \node[right] at (8.7,1) {$b$ represents the line's \textbf{slope}, and is an estimate for the true \textit{slope parameter} $\beta$ (\textit{``beta''})};
    \end{tikzpicture}
\end{center}
...
Another way of writing the equation found above is: $\text{practical}=2.578+0.879\times\text{ theoretical}$. This suggests that each increase of $1$ mark in the theory test is associated with a increase of $0.879$ marks in the practical test.
