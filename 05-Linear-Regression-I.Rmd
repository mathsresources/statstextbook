...
A $y$ *on* $x$ regression model is a one constructed with the aim of producing a trend line which can be used to predict unknown values of $Y$ based on known values of $x$. For a population of bivariate data $(x_1,y_1), (x_2,y_2) ...  (x_n,y_n)$, this *regression line* has an equation of the form $\hat{Y}_i=\alpha + \beta x_i$, where: 
...
\begin{center}
    \begin{tikzpicture}[scale=1]
        ...
        \draw[thick] (0.7,0.7*0.21+1.016) -- (5.6,5.6*0.21+1.016) node[right] {$\hat{Y}_i=\alpha + \beta x_i$};
        \node[right] at (8.7,3) {$\hat{Y}_i$ (\textit{`Y hat'}) is the \textit{best estimate} of $Y_i$ given $x_i$};
        \node[right] at (8.7,2) {$\alpha$ (\textit{``alpha''}) represents the \textit{intercept parameter}};
        \node[right] at (8.7,1) {$\beta$ (\textit{``beta''}) represents the \textit{slope parameter}};
    \end{tikzpicture}
\end{center}
