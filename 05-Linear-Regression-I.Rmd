...
Before applying any statistical analysis techniques to this *bivariate data*, the sports scientist constructs a *scatterplot* to visualise it, looking for any apparent patterns or problems. In this example, $\text{VO}_{\text{2}}$ Max is treated as the **predictor** variable (plotted on the $x$-axis) and 5km running time is treated as the **response** variable (plotted along the $y$-axis). 
...
Whilst the sample size is very small, the scatterplot seems to suggest that there is a *negative linear relationship* between a runner's $\text{VO}_{\text{2}}$ Max and their 5km running time, as higher $\text{VO}_{\text{2}}$ Max values are associated with lower running times. To illustrate this, a *trend line* (or *line of best fit*) has been added to the scatterplot.
...
\textbf{Figure 1} illustrates a *strong positive linear correlation* between $x$ and $y$, where both variables tend to increase together.

\textbf{Figure 2} shows a *weak negative linear correlation* between $x$ and $y$, where, as one variable increases, the other tends to decrease. 
...
Note that there is no universally agreed definition of a 'strong', 'moderate' or 'weak' linear relationship. However, it is still recommended for comments to include some description of the strength of a linear relationship.

Scatterplots should always be constructed and closely scrutinised before any analysis of bivariate data is undertaken. Through this visualisation, it may become apparent, for instance, that the dataset is separated into *distinct groups*, or that possible *outliers* are present which should be carefully considered. 
...
For all sets of data, $-1\leqslant r\leqslant 1$. A correlation coefficient of zero suggests there is *no linear relationship* between the two variables, whilst any non-zero correlation coefficient suggests the possibility of a linear relationship. Each figure below shows a scatterplot of sample data along with its sample correlation coefficient, $r$, and an interpretation. 
...
\end{tikzpicture}
\end{center}
This \textbf{sample} has resulted in a value for $r$ which approximates well the \textcolor{black!50}{\textbf{population}} correlation coefficient, $\rho$.
...
\end{tikzpicture}
\end{center}
This \textbf{sample} has resulted in a value for $r$ which is far from the true \textcolor{black!50}{\textbf{population}} correlation coefficient, $\rho$.
...
A $y$ *on* $x$ regression model is one constructed with the aim of producing a trend line which can be used to predict unknown values of $Y_i$ based on known values of $x_i$. For a sample of bivariate data $(x_1,y_1), (x_2,y_2), ...  ,(x_n,y_n)$, this *regression line* has an equation of the form $\hat{Y}_i=a + b x_i$, where: 
...
Sample statistic $b$ represents the line's \textbf{slope}, and is an estimate for the true \textit{slope parameter} $\beta$ (\textit{``beta'')}. Each 
...
\begin{center}
    \begin{tikzpicture}[scale=1]
        ...
        \draw[fill] (1,1.105) circle (0.04) node[below] {$(x_1,y_1)$};
        \draw[fill] (2,2.17) circle (0.04) node[above] {$(x_2,y_2)$};
        \draw[fill] (3,1.10) circle (0.04) node[below] {$(x_3,y_3)$};
        \draw[fill] (4,1.43) circle (0.04) node[right] {$(x_4,y_4)$};
        \draw[fill] (5,2.47) circle (0.04) node[above] {$(x_5,y_5)$};
...
\node at (9.5,3.5) {$\Sigma x=132,$\quad$\Sigma y=147,$};
\node at (9.5,2.9) {$\Sigma x^2 =1742,$\quad$\Sigma y^2 =2057,$\quad$\Sigma xy=1872$};
...
S_{xx}&=\Sigma x^2-\frac{(\Sigma x)^2}{n}=1742-\frac{132^2}{12}=290&&\\[1em]
S_{xy}&=\Sigma xy-\frac{\Sigma x\Sigma y}{n}=1872-\frac{132\times 147}{12}=255\\[1em]
b&=\dfrac{S_{xy}}{S_{xx}}=\dfrac{255}{290}=0.879\\[1em]
a&=\bar{y}-b \bar{x}=\dfrac{147}{12}-0.879\times \dfrac{132}{12}=2.578\\[1em]
\hat{Y}_i&=2.578+0.879x_i
\end{flalign*}
    \item[(b)] $\hat{Y}=2.578 + 0.879\times 8=9.61$
...
Another way of writing the equation found above is: $\text{practical}=2.578+0.879\times\text{ theoretical}$. This suggests that each increase of $1$ mark in the theory test is associated with an increase of $0.879$ marks in the practical test.
...
\begin{enumerate}
    \item A sample of bivariate data is collected, shown in the table and scatterplot below.
    \begin{center}
        \begin{tabular}{|c|c|c|c|c|c|c|c|c|}
            \hline
        ...
        \end{tabular}
    \end{center}
    The researcher who collected the data constructed a scatterplot, and from that they determined that it would not be appropriate to obtain a least squares regression line. State what feature they are likely to have observed on the scatterplot.
\end{enumerate}
...
\node[right] at (9,2.9) {$\sum x=74.76$};
\node[right] at (9,2.3) {$\sum x^2=253.76$};
\node[right] at (9,1.7) {$\sum y=1408.3$};
\node[right] at (9,1.1) {$\sum y^2=77277$};
\node[right] at (9,0.5) {$\sum xy=4233.1$};
\node[right] at (9,-0.1) {$n=26$};
    \end{tikzpicture}
\end{center}
    \begin{enumerate}
...
\begin{tabular}{c}
  \phantom{-}\\[0.2em]
  $\Sigma x=4673$\\[0.6em]
  $\Sigma x^2=763101$\\[0.6em]
  $\Sigma xy=308020$\\[0.6em]
  $\Sigma y=1889$\\[0.6em]
  $\Sigma y^2=124945$\\[0.6em]
  $n=29$
\end{tabular}
\end{center}
...
\begin{enumerate}[resume]
    ...
    \begin{tikzpicture}[scale=0.8]
        ...
        \node[right] at (7,1.2) {$S_{yy}=530.78$};
        \node[right] at (7,0.5) {$S_{xy}=277.54$};
        \node[right] at (7,-0.2) {$n=17$};
