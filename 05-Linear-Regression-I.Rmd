...
Although the sample size is very small, the scatterplot seems to suggest that there is a *negative linear relationship* between a runner's $\text{VO}_{\text{2}}$ Max and their 5km running time, as higher $\text{VO}_{\text{2}}$ Max values are associated with lower running times. To illustrate this, a **line of best fit** (or *regression line*) has been added to the scatterplot.
...
Note that **correlation does not imply causation**. A linear relationship can exist between variables without implying that one variable causes the other; there may be other variables influencing both. Seemingly linear relationships can also appear by chance - this is referred to as *spurious correlation*.

\begin{tcolorbox}[title=Example,colback=RoyalBlue!1!,colframe=RoyalBlue]
\textbf{Problem:} A French class takes two tests; \textit{listening} and \textit{reading}. The results of the 10 pupils are shown in the table and scatterplot below, along with some summary statistics. Comment on the scatterplot.
...
\node at (2,-1.2) {Listening};
\foreach \y in {8,12,...,20}
...
\draw[fill=black] (9*0.25-1,8*0.25-1.5) circle (0.06);
\draw[fill=black] (7*0.25-1,9*0.25-1.5) circle (0.06);
\draw[fill=black] (11*0.25-1,13*0.25-1.5) circle (0.06);
\draw[fill=black] (20*0.25-1,20*0.25-1.5) circle (0.06);
\draw[fill=black] (4*0.25-1,9*0.25-1.5) circle (0.06);
\draw[fill=black] (6*0.25-1,8*0.25-1.5) circle (0.06);
\draw[fill=black] (17*0.25-1,17*0.25-1.5) circle (0.06);
\draw[fill=black] (12*0.25-1,14*0.25-1.5) circle (0.06);
\draw[fill=black] (10*0.25-1,8*0.25-1.5) circle (0.06);
\draw[fill=black] (15*0.25-1,17*0.25-1.5) circle (0.06);
\end{tikzpicture}
\end{center} 
...
\begin{tcolorbox}[title=Example,colback=RoyalBlue!1!,colframe=RoyalBlue]
\textbf{Problem:} A biologist records the tail length ($x$) and total height ($y$) (in cm) for a random sample of 8 red squirrels. A scatterplot and summary statistics for the data are produced and shown below.
\vspace{-0.5cm}
\begin{center}
\begin{tikzpicture}[scale=0.9]
...
&=\text{2618.3}-\frac{\text{143.4}^\text{2}}{\text{8}}\\
&=\text{47.86}
\end{flalign*}
\vfill
\null
...
&=\text{3391.8}-\frac{\text{163.3}^\text{2}}{\text{8}}\\
&=\text{58.44}
\end{flalign*}
\vfill
\null
...
&=\text{2978.5}-\frac{\text{143.4}\times\text{163.3}}{\text{8}}\\
&=\text{51.35}
\end{flalign*}
\vfill
\null
...
$r=\dfrac{S_{xy}}{\sqrt{S_{xx}S_{yy}}}=\dfrac{\text{51.35}}{\sqrt{\text{47.86}\times\text{58.44}}}=\text{0.971}$\\[0.5em]
...
\node[above,rotate=90] at (-0.1,2.5) {Price (£1000s)};
...
\begin{tcolorbox}[title=Example,colback=RoyalBlue!1!,colframe=RoyalBlue]
\textbf{Problem:} A French class takes two tests; \textit{listening} and \textit{reading}. The results of the 10 pupils are shown in the table and scatterplot below, along with some summary statistics.

\begin{center}
\begin{tabular}{c|cccccccccc}
    Listening ($x$) & 9 & 7 & 11 & 20 & 4 & 6 & 17 & 12 & 10 & 15\\
  \hline
    Reading ($y$) & 8 & 9 & 13 & 20 & 9 & 8 & 17 & 14 & 8 & 17
\end{tabular}
\end{center}

\begin{center}
\begin{tikzpicture}
\draw[semithick] (-0.55,-0.5) -- (-0.55,3.65);
\draw[semithick] (-0.5,-0.6) -- (4.6,-0.6);
\node[rotate=90] at (-1.7,1.75) {Reading};
\node at (2,-1.7) {Listening};
\foreach \y in {6,8,...,20}
\node at (-0.95,0.25*\y-1.5) {\y};
\foreach \y in {6,8,...,20}
\draw[semithick] (-0.55,0.25*\y-1.5) -- (-0.65,0.25*\y-1.5);
\foreach \x in {2,4,...,22}
\node at (0.25*\x-1,-0.95) {\x};
\foreach \x in {2,4,...,22}
\draw[semithick] (0.25*\x-1,-0.6) -- (0.25*\x-1,-0.7);
\draw[semithick] (4*0.25-1,6.45*0.25-1.5) -- (20*0.25-1,19.634*0.25-1.5);
...
\node at (9.5,3.5) {$\Sigma x=\text{111},$\quad$\Sigma y=\text{123},$};
\node at (9.5,2.9) {$\Sigma x^\text{2} =\text{1461},$\quad$\Sigma y^\text{2} =\text{1697},$\quad$\Sigma xy=\text{1554}$};
\node[anchor=west] at (5.5,2) {(a) Calculate the equation of the least squares};
\node[anchor=west] at (6.1,1.5) {regression line of $y$ on $x$.};
\node[anchor=west] at (5.5,0.7) {(b) Calculate a predicted mark for the reading test};
\node[anchor=west] at (6.1,0.2) {for a pupil who missed it, having scored 8 in the};
\node[anchor=west] at (6.1,-0.3) {listening test.};
\node[anchor=west] at (5.5,-1.2) {(c) Comment on the reliability of this prediction.};
\end{tikzpicture}
\end{center}
...
S_{xx}&=\Sigma x^\text{2}-\frac{(\Sigma x)^\text{2}}{n}=\text{1461}-\frac{\text{111}^\text{2}}{\text{10}}=\text{228.9}&&\\[1em]
S_{xy}&=\Sigma xy-\frac{\Sigma x\Sigma y}{n}=\text{1554}-\frac{\text{111}\times \text{123}}{\text{10}}=\text{188.7}\\[1em]
b&=\dfrac{S_{xy}}{S_{xx}}=\dfrac{\text{188.7}}{\text{228.9}}=\text{0.824}\\[1em]
a&=\bar{y}-b \bar{x}=\dfrac{\text{123}}{\text{10}}-\text{0.824}\times \dfrac{\text{111}}{\text{10}}=\text{3.154}\\[1em]
\hat{y}&=\text{3.154}+\text{0.824}x
\end{flalign*}
    \item[(b)] $\hat{y}=\text{3.154} + \text{0.824}\times \text{8}=\text{9.746}$
...
Another way of writing the equation found above is: $\text{reading}=\text{3.154}+\text{0.824}\times\text{ listening}$. 

The estimated slope of $b=\text{0.824}$ suggests that each increase of 1 mark in the listening test is associated with an increase of 0.824 marks in the reading test. The estimated intercept of $a=\text{3.154}$ suggests that for a *listening score of zero*, a predicted reading score should be 3.154. Whilst an interpretation of the slope may often be useful, for many contexts an interpretation for the intercept may be less useful, since an $x$ value of zero will not always be realistic or even possible.
...
\begin{enumerate}[resume]
    \item A researcher laid a transect line from the base of a tree and placed a quadrat at one metre intervals along the transect line. The number of seeds dropped by the tree contained in each quadrat was recorded. 
    \vspace{-0.6cm}
    \begin{center}
    \begin{multicols}{2}
    \null\vfill
...
\begin{enumerate}
    ...
    \begin{enumerate}
        ...
        \end{enumerate}
      \end{enumerate}
      \item[] Based on the correlation coefficient the coach advises that increasing their back squat weight will increase their vertical jump height.
      \begin{enumerate}
...
\node[rotate=90] at (0.3,0.1*180) {Heart rate (bpm)};
\node at (4,14.6) {Age (years)};
...
\begin{enumerate}
    \item[] However, after considering the scatter diagram for the data, it was realised that one piece of data has been misrecorded, and this volunteer’s data (spotted as a possible outlier) was removed from the dataset. 
    \begin{enumerate}
        ...
        \item[(c) ] Calculate the difference this results in for a prediction of the average peak heart rate of a 45 year old volunteer. 
    \end{enumerate}
\end{enumerate}
