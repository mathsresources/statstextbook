...
$r=\dfrac{S_{xy}}{\sqrt{S_{xx}S_{yy}}}=\dfrac{\text{51.39}}{\sqrt{\text{47.82}\times\text{58.43}}}=\text{0.9354}$\\[0.5em]
...
A $y$ *on* $x$ regression model is one constructed with the aim of producing a trend line which can be used to predict unknown values of $y$ based on known values of $x$. For a sample of bivariate data $(x_\text{1},y_\text{1}), (x_\text{2},y_\text{2}), ...  ,(x_n,y_n)$, this *regression line* has an equation of the form $\hat{y}_i=a + b x_i$, where: 

\begin{figure}[H]
  \centering
\begin{minipage}[c]{.54\textwidth}

$\hat{y}_i$ (\textit{`y hat'}) is the \textit{best estimate} of $y_i$ given $x_i$.\\
...
\begin{center}
    \begin{tikzpicture}[scale=1]
        ...
        \draw[thick] (0.7,0.7*0.21+1.016) -- (5.1,5.1*0.21+1.016) node[right] {$\hat{y}_i=a + b x_i$};
    \end{tikzpicture}
\end{center}
...
\hat{y}_i&=\text{2.578}+\text{0.879}x_i
\end{flalign*}
...
\end{center}
\vspace{-0.5cm}
The least squares regression line of $y=\text{155.29}+\text{1.9018}x$ was fitted. Show the calculations required to obtain the values of the sample intercept and sample slope parameter.
\end{enumerate}
