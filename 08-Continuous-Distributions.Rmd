...
\begin{center}
\begin{equation*}
\textbf{pdf: }\quad\frac{1}{\sigma\sqrt{2\pi}}e^{-\dfrac{1}{2}\left(\dfrac{x-\mu}{\sigma}\right)^2}
\end{equation*}
\end{center}
...
\text{P}(Z<z)=&0.999 & \rightarrow z=3.09&\hspace{3cm}
\begin{tikzpicture}[overlay]
\begin{axis}[
  no markers, 
  domain=-3.2:3.2, 
...
Under certain conditions the *discrete* binomial distribution can begin to resemble a *continuous* normal distribution. Given random variable $X\sim \text{B}(n,p)$, **a normal distribution can approximate the binomial distribution**, with *sufficient accuracy*, when:   
...
Approximating probabilities for a *discrete* distribution using a *continuous* distribution may lead to inaccuracy unless a **continuity correction** is applied. For example, consider random variable $X\sim \text{B}(24,0.5)$, with its normal approximation $X\approx \text{N}(12,6)$ overlaid and the *continous* probability $\text{P}(X<14)$ shaded:
...
As with the binomial distribution, under certain conditions the *discrete* Poisson distribution can also begin to resemble a *continuous* normal distribution. Given random variable $X\sim \text{Po}(\lambda)$, **a normal distribution can approximate the Poisson distribution**, with *sufficient accuracy*, when:   
