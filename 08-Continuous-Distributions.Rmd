...
\text{P}(Z<z)=&0.999 & \rightarrow z=3.09&&\\
...
\begin{enumerate}
    \item Given $Z\sim \text{N}(0,1)$, find the value of $z$ if:
    \begin{multicols}{4}
    \begin{enumerate}
...
*Sums and differences* of normally distributed random variables are *themselves normally distributed*. When these random variables are **independent**, the mean and variance of any linear combination of them may be determined using the laws of expectation and variance introduced in section 6.6 For example, given **independent** normally distributed random variables $X$ and $Y$ with means of $\mu_X$ and $\mu_Y$ and standard deviations of $\sigma_X$ and $\sigma_Y$ respectively:
...
\begin{align*}
\text{E}(X_1+X_2+...+X_n)=&\text{E}(X_1)+\text{E}(X_2)+\dots+\text{E}(X_n)=n\text{E}(X)\\
...
\begin{tcolorbox}[title=Example,colback=RoyalBlue!1!,colframe=RoyalBlue]
\textbf{Problem:} The actual volume of paint contained within tins marked as containing 25 litres is normally distributed with a mean of 25.2 litres and a standard deviation of 0.3 litres. Calculate the probability that the total volume in 4 tins is less than 100 litres.
