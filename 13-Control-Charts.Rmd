...
\end{enumerate}
    \item A camera lens manufacturer uses an $x$-bar control chart to monitor the quality of copies of a particular model of lens which it produces. The resolution of the lens is measured using \textit{MTF testing}, where a higher score indicates a higher resolution, which is desired. It is targeting a resolution score of 3600, but the manufacturer knows that the actual resolution of copies of the lens it produces varies, with a standard deviation in resolution score of 120. Samples are taken at regular intervals, and their sample means plotted on the chart are below.
    \begin{center}
    \begin{tikzpicture}[scale=0.8]
        \draw (0,-3.5) rectangle (12,3.5);
        \node at (6,-5) {sample number};
...
\end{center}
    \begin{enumerate}
        \item Describe the feature of the $x$-bar chart which means the process is out of statistical control.
        \item Explain why the lens manufacturers may actually not be concerned with the process.
