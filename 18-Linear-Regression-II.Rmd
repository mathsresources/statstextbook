...
\begin{tcolorbox}[title=The Linear Regression Model,center,colback=red!5,colframe=red!70!black,width=6.5cm]
...
\begin{tcolorbox}[title=Assumptions for the Errors,center,colback=red!5,colframe=red!70!black,width=6.5cm]
\begin{center}
    \begin{itemize}
...
\begin{center}
    \begin{tikzpicture}
        ...
        \node at (10.7,1.55) {\begin{minipage}{6cm}
            \begin{tcolorbox}[title=Calculating a Residual,center,colback=red!5,colframe=red!70!black]
            \vspace{-0.5cm}
\begin{align*}
...
\begin{tcolorbox}[title=Estimating the Error Variance,center,colback=red!5,colframe=red!70!black,width=6cm]
...
\begin{tcolorbox}[title=Coefficient of Determination,center,colback=red!5,colframe=red!70!black]
...
\begin{center}
        ...
        \begin{tcolorbox}[center,colback=red!5,colframe=red!70!black]
\vspace{-0.5cm}
\[R^2=r^2\]
\end{tcolorbox}
        \end{minipage}
...
\begin{minipage}[c]{.33\textwidth}
  \centering
\begin{centering}
\begin{tcolorbox}[title=Test Statistic,center,colback=red!5,colframe=red!70!black,width=5cm,height=2.5cm]
\vspace{-0.3cm}
...
\begin{minipage}[c]{.66\textwidth}
  \centering
\begin{tcolorbox}[title=Conditions,center,colback=red!5,colframe=red!70!black,width=12cm,height=2.5cm]
\begin{itemize}
...
\begin{minipage}[c]{.33\textwidth}
  \centering
\begin{centering}
\begin{tcolorbox}[title=Test Statistic,center,colback=red!5,colframe=red!70!black,width=6cm,height=2.6cm]
...
\begin{minipage}[c]{.66\textwidth}
  \centering
\begin{tcolorbox}[title=Conditions,center,colback=red!5,colframe=red!70!black,width=12cm,height=2.6cm]
\begin{itemize}
...
\begin{tcolorbox}[title=Calculating an $x$ on $y$ Regression Line,center,colback=red!5,colframe=red!70!black,width=12cm]
$\hat{X}=a+by \qquad \text{where} \qquad b=\dfrac{S_{xy}}{S_{yy}} \qquad \text{and} \qquad a=\bar{x}-b\bar{y}$
\end{tcolorbox}

Using this method is an alternative to *relabelling* the variables and *redrawing* a scatterplot.
...
$b=\dfrac{S_{xy}}{S_{yy}}=\dfrac{255}{256.25}=0.995$\\[0.5em]

$a=\bar{x}-b\bar{y}=\dfrac{132}{12}-0.995\times \dfrac{147}{12}=-1.189$\\[0.5em]
...
\begin{tcolorbox}[title=Residual Plots for appropriate models,center,colback=red!5,colframe=red!70!black,width=11cm]
\begin{center}
    \begin{tabular}{ccc}
...
The least squares regression line $\hat{Y}=-5.11+0.5697x$ was fitted to the data, and a scatterplot (\textbf{Figure 1}) and residual plot (\textbf{Figure 2}) drawn.
...
A second linear model was created between speed ($x$) and the square root of the stopping distance ($\sqrt{y}$). For this new model, a scatterplot (\textbf{Figure 3}) and residual plot (\textbf{Figure 4}) were produced, and a least squares regression line was computed using statistical software, with the output below:\\[0.5em] 

\textbf{Output 1:} \texttt{square root of distance = 0.797 + **** $\times$ speed}
...
\begin{enumerate}
    \item[(a)] Comment on the appropriateness of the linear model of $y$ and $x$.
    \item[(b)] Use the data from \textbf{Table 1} to calculate the missing value of \texttt{****} from \textbf{Output 1}.
    \item[(c)] Comment on the suitability of this new model of $\sqrt{y}$ on $x$.
    \item[(d)] Calculate the coefficient of determination for this new model and interpret its value.
\end{enumerate}
...
\begin{tcolorbox}[colback=RoyalBlue!1!, colframe=RoyalBlue]
\textbf{Solution:}

\begin{enumerate}
    \item[(a)] The residual plot for the $y$ on $x$ model appears to show a curved pattern, so this linear model is not suitable.
    \item[(b)] Table of $x$ and $\sqrt{x}$ values:
    \vspace{-0.2cm}
    \begin{center}
    \begin{tabular}{|c|c|c|c|c|c|c|c|c|c|c|}
        \hline
...
\vspace{-0.2cm}
Using a statistical calculator:\newline $\Sigma x=331.5,\;\Sigma x^2=14358.25,\;\Sigma \sqrt{y}=33.83,\;\Sigma \sqrt{y}^2=136.03,\;\Sigma x\sqrt{y}=1385.8,n=10$.

$S_{xx}=14358.25-\frac{331.5^2}{10}=3369.025$ and $S_{x\sqrt{y}}=1385.8-\frac{331.5\times 33.83}{10}=264.34$.

$b=\dfrac{S_{x\sqrt{y}}}{S_{xx}}=\dfrac{264.34}{3369.025}=0.078$ and so $\texttt{****}=0.078$.

The equation of the regression line is: $\sqrt{\texttt{distance}}\texttt{ = 0.797 + 0.78 speed}$ or $\sqrt{y}=0.797+0.78x$.

    \item[(c)] The residual plot for the new model appears to show a random scatter of points centered on zero, with a constant variance as fitted values increase, so the new model seems appropriate.
    \item[(d)] $r=\dfrac{S_{x\sqrt{y}}}{\sqrt{S_{xx}S_{\sqrt{y}\sqrt{y}}}}=\dfrac{264.36}{\sqrt{3369.025\times 21.586}}=0.9803$ and $R^2=0.9803^2=0.961$.\newline
    $96.1$\% of the variation of $\sqrt{\text{distance}}$ is explained by the linear model.
\end{enumerate}
...
\begin{tcolorbox}[center,colback=red!5,colframe=red!70!black,width=12cm]
A $100(1-\alpha)\%$ \textbf{confidence interval} for $\text{E}(Y_i\,|\,x_i)$ is given by:\\[1em]
...
A 99\% CI for $\text{E}(Y\,|\,x=3)$ is $51.75\,\pm\,3.169 \times 17.1\times \sqrt{\dfrac{1}{12}+\dfrac{(3-2.5)^2}{12}}=51.75\,\pm\,17.49$\\[0.3em]
...
\begin{tcolorbox}[center,colback=red!5,colframe=red!70!black,width=12cm]
A $100(1-\alpha)\%$ \textbf{prediction interval} for $Y_i\,|\,x_i$ is given by:\\[1em]
