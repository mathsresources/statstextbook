...
$\varepsilon$ (*"epsilon"*) is a random variable called the \textbf{error} that describes how far $Y$ is from its expected value. 

$\alpha$ and $\beta$ are fixed parameters whose value cannot be known exactly without examining all possible occurrences of $Y$ and $x$. However the sample observations $(x_i,y_i)$ can be used to give estimates $a$ and $b$ of $\alpha$ and $\beta$. This leads to the relationship $\hat{Y}_i=a+bx_i$, where $\hat{Y}_i$ (*"y-hat"*) denotes the predicted value of $Y_i$ for a given $x_i$.
...
The above estimates rely on several assumptions about the population of errors, $\varepsilon_i$:

\begin{tcolorbox}[center,colback=red!5,colframe=red!70!black,width=7cm]
\begin{center}
    \begin{itemize}
...
Formulae for the estimates of $\alpha$ and $\beta$ and the assumptions about the errors are given on Page 5 of the Data Booklet.
...
\begin{center}
    \begin{tikzpicture}
        ...
        \node at (10.7,1.55) {\begin{minipage}{5cm}
...
For data points which lie \textit{above} the regression line, their residual is \textit{positive}.\newline For those which lie \textit{below} the line, their residual is \textit{negative}.

To calculate a residual the fitted value, $\hat{Y}_i$ needs to first be found.
...
\draw[fill,RoyalBlue] (10*0.2,8*0.12) circle (0.05);
\draw[thick] (10*0.2,8*0.12) -- (10*0.2,11.6*0.12);
\draw[fill=black] (15*0.2,17*0.12) circle (0.02);
\draw[fill,RoyalBlue] (16*0.2,18*0.12) circle (0.05);
    \end{tikzpicture}
\end{center}
...
An assumption made for the least squares regression model is that the variance of the errors is a constant for all $x_i$ (and $Y_i$):
...
\begin{tcolorbox}[center,colback=red!5,colframe=red!70!black,width=3cm]
\vspace{-0.5cm}
\[s^2=\dfrac{SSR}{n-2}\]
\end{tcolorbox}
